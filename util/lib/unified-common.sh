#!/bin/bash
#==============================================================================
# Filename: unified-common.sh
# Description: Unified common utility functions for Samstraumr bash scripts
#
# This library consolidates utility functions from multiple common libraries
# into a single, comprehensive utility library for all Samstraumr bash scripts.
# It ensures consistent formatting, error handling, and common operations
# across the project while eliminating duplication.
#
# USAGE:
...
#
# DEPENDENCIES:
#   - Requires standard Unix utilities (sed, grep, tr)
#==============================================================================
...
if [ -f "${PROJECT_ROOT}/.samstraumr.config" ]; then
  ...
  echo "Warning: Configuration file not found: ${PROJECT_ROOT}/.samstraumr.config or ${PROJECT_ROOT}/.samstraumr/config.sh"
...
readonly NC="${COLOR_RESET}"  # No Color (alias)
...
# Legacy function aliases for compatibility with older scripts
function info() { print_info "$1"; }
function success() { print_success "$1"; }
function error() { print_error "$1"; exit 1; }
function warn() { print_warning "$1"; }
function highlight() { echo -e "${COLOR_CYAN}$1${COLOR_RESET}"; }
function important() { echo -e "${COLOR_MAGENTA}$1${COLOR_RESET}"; }
...
# Run Maven with timing information and proper arguments
# Usage: run_maven arg1 arg2 ...
# Returns: Maven's exit code
function run_maven() {
  local cmd=("mvn")
...
# Setup Maven options with memory settings
# Usage: setup_maven_opts
# Side effects: Sets MAVEN_OPTS environment variable
function setup_maven_opts() {
  MVN_OPTS=""
...
# Usage: check_requirements git mvn java
# Returns: 0 if all requirements are met, exits with code 1 otherwise
function check_requirements() {
  local missing=0
  
  for cmd in "$@"; do
    if ! command_exists "$cmd"; then
      echo -e "${COLOR_RED}Required command not found: ${cmd}${COLOR_RESET}" >&2
      missing=1
    fi
  done
  
  if [ $missing -eq 1 ]; then
    echo -e "${COLOR_RED}Please install the missing requirements and try again.${COLOR_RESET}" >&2
    exit 1
  fi
...
function time_exec() {
  ...
  if [ $duration -gt 60 ]; then
    local minutes=$((duration / 60))
    local seconds=$((duration % 60))
    echo -e "${COLOR_YELLOW}Execution time: ${minutes}m ${seconds}s${COLOR_RESET}"
  else
    echo -e "${COLOR_YELLOW}Execution time: ${duration}s${COLOR_RESET}"
  fi
...
function initialize_common_library() {
  ...
  if [[ "${VERBOSE:-false}" == "true" ]]; then
    print_debug "Loaded unified-common.sh library"
