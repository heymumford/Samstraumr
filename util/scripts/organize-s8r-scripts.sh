...
for script in s8r s8r-init s8r-list; do
  if [[ -f "$BASE_DIR/$script" ]]; then
    # Create a symbolic link in the bin/core directory
    ln -sf "$BASE_DIR/$script" "$BASE_DIR/bin/core/$script"
    print_message "Created symlink bin/core/$script -> $script"
  fi
done
...
for script in s8r-build s8r-build-new s8r-ci; do
  if [[ -f "$BASE_DIR/$script" ]]; then
    # Create a symbolic link in the bin/build directory
    ln -sf "$BASE_DIR/$script" "$BASE_DIR/bin/build/$script"
    print_message "Created symlink bin/build/$script -> $script"
  fi
done
...
for script in s8r-test s8r-test-new s8r-test-cli s8r-run-tests.sh; do
  if [[ -f "$BASE_DIR/$script" || -L "$BASE_DIR/$script" ]]; then
    # Create a symbolic link in the bin/test directory
    ln -sf "$BASE_DIR/$script" "$BASE_DIR/bin/test/$script"
    print_message "Created symlink bin/test/$script -> $script"
  fi
done
...
for script in s8r-component s8r-composite s8r-machine s8r-tube; do
  if [[ -f "$BASE_DIR/$script" ]]; then
    # Create a symbolic link in the bin/component directory
    ln -sf "$BASE_DIR/$script" "$BASE_DIR/bin/component/$script"
    print_message "Created symlink bin/component/$script -> $script"
  fi
done
...
for script in s8r-dev s8r-dev-help-build; do
  if [[ -f "$BASE_DIR/$script" ]]; then
    # Create a symbolic link in the bin/dev directory
    ln -sf "$BASE_DIR/$script" "$BASE_DIR/bin/dev/$script"
    print_message "Created symlink bin/dev/$script -> $script"
  fi
done
...
for script in s8r-migration-monitor s8r-migration-report s8r-architecture-verify s8r-structure-verify; do
  if [[ -f "$BASE_DIR/$script" ]]; then
    # Create a symbolic link in the bin/migration directory
    ln -sf "$BASE_DIR/$script" "$BASE_DIR/bin/migration/$script"
    print_message "Created symlink bin/migration/$script -> $script"
  fi
done
...
for script in s8r-help s8r-help-build s8r-help-component; do
  if [[ -f "$BASE_DIR/$script" ]]; then
    # Create a symbolic link in the bin/help directory
    ln -sf "$BASE_DIR/$script" "$BASE_DIR/bin/help/$script"
    print_message "Created symlink bin/help/$script -> $script"
  fi
done
...
for script in s8r-docs s8r-quality s8r-version s8r-version-robust s8r-version-simple; do
  if [[ -f "$BASE_DIR/$script" || -L "$BASE_DIR/$script" ]]; then
    # Create a symbolic link in the bin/utils directory
    ln -sf "$BASE_DIR/$script" "$BASE_DIR/bin/utils/$script"
    print_message "Created symlink bin/utils/$script -> $script"
  fi
done
...
# Create the bin/s8r-all directory symlink for easy access to all scripts
...
for script in $(find "$BASE_DIR" -maxdepth 1 -name "s8r*" -type f -o -name "s8r*" -type l); do
  ...
  ln -sf "$BASE_DIR/$script_name" "$BASE_DIR/bin/s8r-all/$script_name"
  print_message "Created symlink bin/s8r-all/$script_name -> $script_name"
done
...
All scripts in this directory are symbolic links to the actual script files in the project root. This organization helps maintain backward compatibility while providing a more organized structure.
EOF
