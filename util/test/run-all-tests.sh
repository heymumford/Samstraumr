...
MAP_SCRIPT="$SCRIPT_DIR/mapping/map-test-type.sh"

if [ "$INCLUDE_EQUIVALENT" = true ]; then
  CMD="mvn test -P $MAVEN_PROFILE -Dcucumber.filter.tags=\"@$TEST_TYPE or @$($MAP_SCRIPT $TEST_TYPE)\""
else
