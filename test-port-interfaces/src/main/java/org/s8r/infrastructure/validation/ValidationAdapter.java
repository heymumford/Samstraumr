 ...
 *
 * <p>This adapter provides validation operations for the application using a simple rules-based
 * validation system. It follows Clean Architecture principles by implementing the port interface
...
public class ValidationAdapter implements ValidationPort {
  ...
  /**
   * Creates a new instance of ValidationAdapter.
   * This is a factory method primarily for Karate tests.
   *
   * @return A new ValidationAdapter instance
   */
  public static ValidationAdapter createInstance() {
    return new ValidationAdapter();
  }

  /**
   * Constructs a new ValidationAdapter with standard rules.
   */
  ...
  private void initializeStandardRules() {
    ...
    rules.put(
        ...
        (ValidationRule<String>)
            value -> value != null && value.length() >= 3 && value.length() <= 50);
  ...
  @Override
  public ValidationResult validateAllowedValues(
    ...
    return ValidationResult.invalid(
        "Field "
            + fieldName
            + " must be one of: "
            + String.join(", ", allowedValues.stream().map(Object::toString).toList()));
  }
  ...
  public void registerRule(String ruleName, Object ruleDefinition) {
    if (ruleName != null && ruleName.startsWith("pattern:")) {
      // If the rule definition is a pattern string, create a Pattern object
      String patternString = ruleName.substring("pattern:".length());
      rules.put(ruleName, Pattern.compile(patternString));
    } else {
   ...
   *
   * @return A map of all registered rules
   */
   ...
   *
   * @return A map of all registered rule sets
   */
  public Map<String, Map<String, Object>> getAllRuleSets() {
    return new HashMap<>(ruleSets);
  }

  /**
   * Gets all registered entity validators.
   *
   * @return A map of all registered entity validators
   */
  public Map<String, Map<String, Object>> getAllEntityValidators() {
    return new HashMap<>(entityValidators);
  }
