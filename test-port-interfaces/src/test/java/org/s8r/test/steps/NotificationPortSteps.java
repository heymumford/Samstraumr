...
public class NotificationPortSteps {
    ...
    public void theNotificationShouldHaveSubject(String subject) {
        // For the test Scenario: Send a basic notification, we need to check the notifications in the adapter
        // since we don't have a listener in place for this scenario
        
        // Get notifications for the test recipient
        List<Map<String, Object>> notifications = notificationAdapter.getNotificationsFor("recipient@example.com");
        boolean found = false;
        
        for (Map<String, Object> notification : notifications) {
            if (notification.get("subject").equals(subject)) {
                found = true;
                break;
            }
        }
        
        Assertions.assertTrue(found, "Notification with subject '" + subject + "' should be found");
    }
