...
public class CacheFileSystemIntegrationSteps {
    ...
    public void iClearTheCacheForIntegrationTest() {
        cachePort.clear();
    }
    ...
    public void iLoadTheCacheFromFile(String filename) throws IOException {
        ...
        Optional<String> content = fileSystemPort.readFile(filePath);
        if (content.isPresent()) {
            // Simple parsing of our basic format
            ...
            for (String line : lines) {
                if (line.contains("=")) {
                    ...
                    // Special handling for complex object
                    if (key.equals("complex-key")) {
    ...
    @Then("the cache should not contain integration key {string}")
    public void theCacheShouldNotContainIntegrationKey(String key) {
        Assertions.assertFalse(cachePort.containsKey(key), "Cache should not contain key: " + key);
    }
    
    @Then("the cache should contain integration key {string}")
    public void theCacheShouldContainIntegrationKey(String key) {
        Assertions.assertTrue(cachePort.containsKey(key), "Cache should contain key: " + key);
    }
    
    @Then("the integration value for key {string} should be {string}")
    public void theIntegrationValueForKeyShouldBe(String key, String expectedValue) {
    ...
    @When("I put a complex object in integration cache with key {string}")
    public void iPutAComplexObjectInIntegrationCacheWithKey(String key) {
