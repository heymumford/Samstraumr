...
/**
 * Mock implementation of the ValidationPort interface for testing.
 */
public class MockValidationAdapter implements ValidationPort {

    private final LoggerPort logger;
    private final Map<String, Object> rules = new ConcurrentHashMap<>();
    private final Map<String, Map<String, Object>> ruleSets = new ConcurrentHashMap<>();
    private final Map<String, Map<String, Object>> entityValidators = new ConcurrentHashMap<>();

    /**
     * Creates a new MockValidationAdapter.
     *
     * @param logger the logger to use
     */
    public MockValidationAdapter(LoggerPort logger) {
        this.logger = logger;
    ...
    private void initializeStandardRules() {
        // String rules
        rules.put("non-empty", Pattern.compile(".+"));
        rules.put("email", Pattern.compile("^[\\w-\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$"));
        rules.put("alphanumeric", Pattern.compile("^[a-zA-Z0-9]*$"));
        rules.put("numeric", Pattern.compile("^[0-9]*$"));
        rules.put("alphabetic", Pattern.compile("^[a-zA-Z]*$"));
        ...
        logger.debug("MockValidationAdapter initialized with standard rules");
    }
    ...
    public void registerRule(String ruleName, Object ruleDefinition) {
        ...
        if (ruleName.equals("uppercase") && ruleDefinition instanceof String) {
            // Special case for the test scenario
            rules.put(ruleName, Pattern.compile((String) ruleDefinition));
        } else {
