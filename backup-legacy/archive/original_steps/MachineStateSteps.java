 ...
 *
 * <p>Note: The Machine class is not yet implemented, so this uses a simulation of the machine
 * behavior using existing Tube components.
 */
public class MachineStateSteps {
  ...
  public void each_bundle_should_have_its_own_state() {
    ...
    for (String bundleName : bundles.keySet()) {
      assertTrue(
          bundleStates.containsKey(bundleName), "Bundle '" + bundleName + "' should have state");

      assertNotNull(
          bundleStates.get(bundleName), "Bundle '" + bundleName + "' state should not be null");

      assertEquals(
          "READY",
          bundleStates.get(bundleName).get("status"),
  ...
  public void the_state_hierarchy_should_be_correctly_established() {
    ...
    assertEquals(bundles.size(), bundleStates.size(), "Machine should track state for all bundles");
