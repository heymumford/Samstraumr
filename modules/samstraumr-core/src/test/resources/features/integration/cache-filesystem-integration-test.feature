...
Feature: Cache and FileSystem Integration
  ...
  Scenario: Cache data is persisted to the file system
    ...
    Then the cache should not contain key "test-key"
    When I load the cache from file "cache-data.json"
    Then the cache should contain key "test-key"
    And the value for key "test-key" should be "test-value"
