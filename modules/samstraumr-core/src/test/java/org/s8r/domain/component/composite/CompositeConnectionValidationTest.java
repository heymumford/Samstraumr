...
class CompositeConnectionValidationTest {
  ...
  void setUp() {
    ...
    composite =
        CompositeComponent.create(ComponentId.create("TestComposite"), CompositeType.STANDARD);
  ...
  void validConnectionsShouldBeCreatedSuccessfully() {
    ...
    ComponentConnection connection =
        composite.connect(
            componentA.getId(), componentB.getId(), ConnectionType.DATA_FLOW, "Test connection");
  ...
  void connectionCreatingDirectCycleShouldThrow() {
    ...
    composite.connect(componentA.getId(), componentB.getId(), ConnectionType.DATA_FLOW, "A to B");

    // Attempt to create B -> A, which would form a cycle
    ConnectionCycleException exception =
        assertThrows(
            ConnectionCycleException.class,
            () ->
                composite.connect(
                    componentB.getId(), componentA.getId(), ConnectionType.DATA_FLOW, "B to A"));
  ...
  void connectionCreatingIndirectCycleShouldThrow() {
    ...
    composite.connect(componentB.getId(), componentC.getId(), ConnectionType.DATA_FLOW, "B to C");
    ...
    ConnectionCycleException exception =
        assertThrows(
            ...
            () ->
                composite.connect(
                    componentC.getId(), componentA.getId(), ConnectionType.DATA_FLOW, "C to A"));
  ...
  void peerConnectionsShouldNotCreateCycles() {
    ...
    composite.connect(componentA.getId(), componentB.getId(), ConnectionType.PEER, "A to B peer");

    // Create B -> A peer connection - should not throw exception
    ComponentConnection connection =
        composite.connect(
            componentB.getId(), componentA.getId(), ConnectionType.PEER, "B to A peer");
