...
/**
 * Tests to verify integration of the architecture documentation tools with the build process and
...
public class ArchitectureToolsIntegrationTest {
  ...
  void testDirectoryStructure() {
    ...
    assertTrue(Files.exists(docsDir) && Files.isDirectory(docsDir), "docs directory should exist");
    assertTrue(
        Files.exists(architectureDir) && Files.isDirectory(architectureDir),
        "architecture directory should exist");
    assertTrue(
        Files.exists(decisionsDir) && Files.isDirectory(decisionsDir),
  ...
  void testScriptsInBinDirectory() {
    ...
    assertTrue(Files.exists(binDir) && Files.isDirectory(binDir), "bin directory should exist");

    Path generateDiagramsScript = binDir.resolve("generate-diagrams.sh");
    Path newAdrScript = binDir.resolve("new-adr");

    assertTrue(Files.exists(generateDiagramsScript), "generate-diagrams.sh script should exist");
    assertTrue(Files.exists(newAdrScript), "new-adr script should exist");
  }
  ...
  void testCoreMavenDependencies() throws Exception {
    ...
    assertTrue(pomContent.contains("junit-jupiter"), "POM should include JUnit Jupiter dependency");
    assertTrue(
        pomContent.contains("junit-platform-suite")
    ...
    assertTrue(
        pomContent.contains("cucumber-java") || pomContent.contains("cucumber-junit"),
  ...
  void testArchitectureDecisionRecords() throws Exception {
    ...
    List<String> expectedAdrPrefixes =
        Arrays.asList(
    ...
    for (String expectedPrefix : expectedAdrPrefixes) {
      boolean found = actualAdrFiles.stream().anyMatch(file -> file.startsWith(expectedPrefix));
    ...
    for (String adrFile : actualAdrFiles) {
      ...
      if (!title.isEmpty()) {
        ...
        assertTrue(
            indexContent.contains(titleText), "ADR index should reference ADR: " + titleText);
      }
    }
  }
  ...
  void testDiagramGenerationOnBuild() throws Exception {
    ...
    if (Files.exists(pythonScript)) {
      assertTrue(Files.isExecutable(pythonScript), "Python diagram script should be executable");
    }
  }
