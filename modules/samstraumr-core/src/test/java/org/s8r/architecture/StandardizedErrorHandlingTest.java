...
public class StandardizedErrorHandlingTest {
  ...
  void exceptionHierarchyShouldBeProperlyStructured() {
    ...
    DuplicateComponentException duplicateEx =
        new DuplicateComponentException("Duplicate component", testId);

    // Check inheritance
    assertTrue(
        notFoundEx instanceof org.s8r.domain.exception.ComponentException,
        "ComponentNotFoundException should extend ComponentException");
    assertTrue(
        duplicateEx instanceof org.s8r.domain.exception.ComponentException,
        "DuplicateComponentException should extend ComponentException");

    // Check component ID is preserved
    assertEquals(
        testId,
        notFoundEx.getComponentId(),
        "ComponentNotFoundException should preserve ComponentId");
    assertEquals(
        testId,
        duplicateEx.getComponentId(),
        "DuplicateComponentException should preserve ComponentId");
  }
  ...
  void compositeExceptionsShouldHandleMultipleErrors() {
    ...
    assertTrue(
        suppressed[0] instanceof ComponentNotFoundException,
  ...
  void exceptionsShouldBeUnchecked() {
    ...
    assertTrue(
        domainEx instanceof RuntimeException, "Domain exceptions should extend RuntimeException");
    assertTrue(
        componentEx instanceof RuntimeException,
  ...
  void domainAndCoreExceptionsAreProperlyStructured() {
    ...
    assertNotEquals(
        org.s8r.domain.exception.ComponentException.class,
        InitializationException.class,
        "Core exceptions should be separate from domain exceptions");
    assertNotEquals(
        org.s8r.component.ComponentException.class,
        InitializationException.class,
        "Core exceptions should be separate from component exceptions");
    assertTrue(
        domainEx instanceof org.s8r.domain.exception.ComponentException,
  ...
  void exceptionsShouldHaveMeaningfulMessages() {
    ...
    assertTrue(
        message.contains(testId.toString()) || message.contains(testId.getShortId()),
