 ...
 *
 * <p>This test class verifies that any implementation of the ComponentPort interface adheres to the
 ...
 *
 * <p>The tests cover core functionality such as identity, lifecycle management, domain events, and
 * data publishing.
 */
public class ComponentPortContractTest extends PortContractTest<ComponentPort> {
  ...
  protected ComponentPort createPortImplementation() {
    ...
    domainComponent =
        Component.create(org.s8r.domain.identity.ComponentId.create("test-component"));
  ...
  public void basicIdentityTests() {
    ...
    assertEquals(
        "Component", portUnderTest.getType(), "Default component type should be 'Component'");

    // The state should reflect the lifecycle state
    assertEquals(
        portUnderTest.getLifecycleState().name(),
        portUnderTest.getState(),
    ...
    assertTrue(
        portUnderTest.getCreationTime().isBefore(Instant.now()),
  ...
  public void lifecycleManagementTests() {
    ...
    assertEquals(
        ...
        portUnderTest.getLifecycleState(),
        "Initial state should be INITIALIZED");
  ...
  public void domainEventTests() {
    ...
    assertTrue(portUnderTest.getDomainEvents().isEmpty(), "Initial domain events should be empty");
    ...
    boolean foundStateChangedEvent =
        events.stream().anyMatch(e -> e instanceof ComponentStateChangedEvent);
    assertTrue(foundStateChangedEvent, "Should contain ComponentStateChangedEvent");

    // Clear events
    portUnderTest.clearEvents();
    assertTrue(
        portUnderTest.getDomainEvents().isEmpty(), "Domain events should be empty after clear");
  }
  ...
  public void dataPublishingTests() {
    ...
    boolean foundDataEvent = events.stream().anyMatch(e -> e instanceof ComponentDataEvent);
    ...
    events.stream()
        ...
        .forEach(
            e -> {
              assertNotNull(e.getSourceId(), "SourceId should not be null");
              assertNotNull(e.getDataChannel(), "DataChannel should not be null");
  ...
  public void lineageManagementTests() {
    ...
    assertTrue(
        updatedLineage.size() >= initialLineage.size() + 2,
