 ...
 *
 * <p>This test class verifies that any implementation of the DataFlowEventPort interface adheres to
 ...
 *
 * <p>The tests cover core functionality such as subscribing, unsubscribing, and publishing data to
 * channels.
 */
public class DataFlowEventPortContractTest extends PortContractTest<DataFlowEventPort> {
  ...
  public void subscriptionManagementTests() {
    ...
    assertTrue(
        portUnderTest.getComponentSubscriptions(sourceId).isEmpty(),
    ...
    assertFalse(
        subscriptions.isEmpty(), "Component subscriptions should not be empty after subscribe");
    assertTrue(
        subscriptions.contains("test-channel"), "Component should be subscribed to test-channel");
    ...
    assertTrue(
        subscriptions.contains("test-channel-2"),
        "Component should still be subscribed to test-channel-2");
    ...
    subscriptions = portUnderTest.getComponentSubscriptions(sourceId);
    assertTrue(
        subscriptions.isEmpty(), "Component should have no subscriptions after unsubscribeAll");
  }
    ...
    doAnswer(
            invocation -> {
              ...
              assertEquals(
                  ...
                  "Event channel should match published channel");
              assertEquals(testData, event.getData(), "Event data should match published data");
              return null;
            })
        .when(capturingHandler)
        .accept(any(ComponentDataEvent.class));
  ...
  public void multipleSubscribersTests() {
    ...
    ComponentId thirdId = ComponentId.create("Third component for testing");
