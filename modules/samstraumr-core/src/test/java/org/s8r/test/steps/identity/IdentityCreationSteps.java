...
public class IdentityCreationSteps {
  ...
  public void the_identity_should_include_a_creation_timestamp() {
    ...
    Assertions.assertTrue(
        creationTime.isBefore(now) || creationTime.isEqual(now),
        "Creation timestamp should not be in the future");

    LocalDateTime oneHourAgo = now.minusHours(1);
    Assertions.assertTrue(
        creationTime.isAfter(oneHourAgo),
  ...
  public void the_identity_should_match_the_pattern(String pattern) {
    ...
    String regexPattern =
        pattern
            ...
            .replace(
                "{parent-uuid}", "[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}");

    Assertions.assertTrue(
        identityStr.matches(regexPattern),
  ...
  public void the_identity_should_include_the_parents_identity_as_prefix() {
    ...
    Assertions.assertTrue(
        childIdentityStr.startsWith(parentIdentityStr),
  ...
  public void the_bottom_component_should_have_a_valid_identity() {
    ...
    Assertions.assertNotNull(
        grandchildComponent.getIdentity(), "Bottom-level component should have identity");
  }
  ...
  public void the_identity_should_be_a_grandchild_identity() {
    ...
    Assertions.assertEquals(2, childCount, "Grandchild identity should have 2 '/child/' sections");
  }
  ...
  public void the_identity_should_contain_both_parent_and_grandparent_references() {
    ...
    Assertions.assertTrue(
        grandchildIdentityStr.startsWith(grandparentIdentityStr),
        "Grandchild identity should start with grandparent identity");

    Assertions.assertTrue(
        grandchildIdentityStr.startsWith(parentIdentityStr),
  ...
  public void each_components_identity_should_contain_references_to_all_its_ancestors() {
    ...
    for (int i = 1; i < components.size(); i++) {
      ...
      Assertions.assertTrue(
          identityStr.startsWith(parentIdentityStr),
  ...
  public void the_leaf_components_identity_should_have_n_sections(int count, String section) {
    ...
    Assertions.assertEquals(
        count, sectionCount, "Leaf identity should have " + count + " '" + section + "' sections");
  }
  ...
  public void all_n_components_should_have_unique_identities(int count) {
    Assertions.assertEquals(
        count, components.size(), "Should have created " + count + " components");
    ...
    for (Component component : components) {
      ...
      Assertions.assertFalse(
          identities.containsKey(identityStr), "Component identities should be unique");
  ...
  public void identity_creation_performance_should_be_within_acceptable_limits() {
    ...
    Assertions.assertTrue(
        avgTimePerComponent <= threshold,
        "Average identity creation time should be <= "
            + threshold
            + "ms, but was "
            + avgTimePerComponent
            + "ms");
  ...
  public void identities_should_be_correctly_structured() {
    for (Component component : components) {
      ...
      if (identity.isAdam()) {
        Assertions.assertTrue(
            identityStr.startsWith("adam/"), "Adam identity should start with 'adam/'");
      } else {
        Assertions.assertTrue(
            identityStr.contains("/child/"), "Child identity should contain '/child/'");
      }
    }
  }
  ...
  public void each_identity_should_be_valid() {
    for (Component component : components) {
      ...
      Assertions.assertTrue(identity.isValid(), "Each identity should be valid");
    }
  }
  ...
  public void the_deserialized_identity_should_match_the_original_identity() {
    ...
    Assertions.assertEquals(
        serializedIdentity.toString(),
        deserializedIdentity.toString(),
