...
public class LifecycleStateMachineSteps {
  ...
  public void the_component_should_transition_through_the_following_states_in_order(
      DataTable dataTable) {
    List<String> expectedStateNames = dataTable.asList();
    List<State> expectedStates =
        expectedStateNames.stream().map(State::valueOf).collect(Collectors.toList());
    ...
    for (int i = 0; i < expectedStates.size(); i++) {
      ...
      Assertions.assertTrue(found, "Component should have transitioned through state: " + expected);
    }
  }
  ...
  public void the_components_memory_log_should_record_all_state_transitions() {
    ...
    Assertions.assertTrue(
        foundStateChanges, "Component memory log should record state transitions");
  }
  ...
  public void the_component_should_be_operational() {
    Assertions.assertTrue(component.isOperational(), "Component should be operational");
  }

  @Then("the component cannot transition directly to the following states:")
  public void the_component_cannot_transition_directly_to_the_following_states(
      DataTable dataTable) {
    ...
    for (String stateName : invalidStateNames) {
      ...
      try {
        ...
        Assertions.fail(
            "Component should not be able to transition from "
                + currentState
                + " to "
                + targetState);
  ...
  public void the_component_can_transition_to_the_following_states(DataTable dataTable) {
    ...
    for (String stateName : validStateNames) {
      ...
      try {
        ...
        Assertions.fail(
            "Component should be able to transition to "
                + stateName
                + " but got: "
                + e.getMessage());
      }
  ...
  public void the_transition_should_be_rejected() {
    Assertions.assertNotNull(lastException, "Invalid transition should throw an exception");
  }
  ...
  public void an_invalid_state_transition_exception_should_be_thrown() {
    Assertions.assertTrue(
        lastException instanceof InvalidStateTransitionException,
  ...
  public void the_component_can_transition_to_state(String stateName) {
    try {
      ...
      Assertions.fail(
          "Component should be able to transition to " + stateName + " but got: " + e.getMessage());
    }
  }
  ...
  public void the_component_should_preserve_knowledge_before_termination() {
    ...
    Assertions.assertTrue(
        foundPreservationEntry, "Component should preserve knowledge before termination");
  }
  ...
  public void the_component_should_release_all_resources() {
    ...
    Assertions.assertTrue(
        foundResourceReleaseEntry, "Component should release all resources during termination");
  ...
  public void the_component_should_unsubscribe_from_all_events() {
    ...
    Assertions.assertTrue(
        foundUnsubscribeEntry, "Component should unsubscribe from all events during termination");

    // Check that there are no active listeners
    List<?> activeListeners = component.getActiveListeners();
    Assertions.assertTrue(
        activeListeners.isEmpty(), "Component should have no active listeners after termination");
  }
  ...
  public void a_component_in_state_with_active_connections(String stateName) {
    ...
    Assertions.assertEquals(
        2, component.getResourceUsage("connections"), "Component should have 2 active connections");
  }
  ...
  public void data_processing_should_be_paused() {
    ...
    Assertions.assertFalse(
        component.getState().allowsDataProcessing(),
  ...
  public void the_component_should_store_the_pre_suspension_state() {
    ...
    Assertions.assertNotNull(preSuspendedState, "Component should store the pre-suspension state");
    Assertions.assertEquals(
        preTransitionState.name(),
        preSuspendedState,
  ...
  public void advanced_configuration_operations_should_be_allowed() {
    Assertions.assertTrue(
        component.getState().allowsAdvancedConfigurationChanges(),
  ...
  public void i_perform_advanced_configuration_changes() {
    try {
      ...
      Assertions.fail(
          "Should be able to perform advanced configuration changes: " + e.getMessage());
    }
  }
  ...
  public void the_new_configuration_should_be_applied() {
    ...
    Assertions.assertEquals(
        "new-value1",
        env.getParameter("advanced.setting1"),
        "Advanced setting 1 should be updated");
    Assertions.assertEquals(
        "new-value2",
        env.getParameter("advanced.setting2"),
  ...
  public void resource_usage_should_be_restored() {
    ...
    Assertions.assertTrue(
        component.getResourceUsage("memory") > 0,
        "Memory resource should be restored after recovery");
    Assertions.assertTrue(
        component.getResourceUsage("threads") > 0,
  ...
  public void the_recovery_attempt_should_be_logged() {
    ...
    Assertions.assertTrue(foundRecoveryEntry, "Component should log recovery attempts");
  }
  ...
  public void resource_levels_should_change_appropriately_for_each_state() {
    ...
    if (resourceLevels.containsKey("ACTIVE.memory")) {
      Assertions.assertTrue(
          (int) resourceLevels.get("ACTIVE.memory") >= (int) resourceLevels.get("initial.memory"),
    ...
    if (resourceLevels.containsKey("SUSPENDED.connections")) {
      Assertions.assertEquals(
          0,
          resourceLevels.get("SUSPENDED.connections"),
    ...
    if (resourceLevels.containsKey("MAINTENANCE.connections")) {
      Assertions.assertEquals(
          0,
          resourceLevels.get("MAINTENANCE.connections"),
    ...
    if (resourceLevels.containsKey("TERMINATED.memory")) {
      Assertions.assertTrue(
          (int) resourceLevels.get("TERMINATED.memory")
              < (int) resourceLevels.get("initial.memory"),
  ...
  public void no_resource_leaks_should_occur() {
    ...
    if (component.getState() == State.TERMINATED) {
      ...
      Assertions.assertEquals(
          0,
          component.getResourceUsage("timers"),
          "All timers should be released in TERMINATED state");
    }
  }

  @Then("the following states should be in the {string} category:")
  public void the_following_states_should_be_in_the_category(
      String categoryName, DataTable dataTable) {
    List<String> stateNames = dataTable.asList();
    ...
    for (String stateName : stateNames) {
      ...
      switch (categoryName) {
        case "OPERATIONAL":
          Assertions.assertTrue(
              state.isOperational(), state + " should be in OPERATIONAL category");
          break;
        case "LIFECYCLE":
          Assertions.assertTrue(state.isLifecycle(), state + " should be in LIFECYCLE category");
          break;
        case "ADVANCED":
          Assertions.assertTrue(state.isAdvanced(), state + " should be in ADVANCED category");
          break;
        case "TERMINATION":
          Assertions.assertTrue(
              state.isTermination(), state + " should be in TERMINATION category");
          break;
        default:
