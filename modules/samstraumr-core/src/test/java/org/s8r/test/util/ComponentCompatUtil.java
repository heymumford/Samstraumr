 ...
 *
 * <p>This class helps migrate tests from old Component API patterns to new ones without requiring
...
public class ComponentCompatUtil {
  ...
  /**
   * Converts a string state name to a State enum value. Handles both new State names and legacy
   * LifecycleState names.
   *
  ...
  public static State convertState(String stateName) {
    ...
    try {
      ...
      switch (stateName) {
        ...
        case "ERROR_PHASE":
          return State.ERROR;
        case "RECOVERING_PHASE":
          return State.RECOVERING;
        case "TERMINATING_PHASE":
        ...
        default:
          return State.READY; // Default to READY if unknown
      }
    }
  }
   ...
   */
  public static Map<String, Object> executeOperation(
      Component component, String operation, Map<String, Object> parameters) {
    // Check if operation is allowed in current state
    if (!hasCapability(component, operation)) {
      throw new IllegalStateException(
          "Operation " + operation + " not allowed in state " + component.getState());
    }
  ...
  public static boolean validateProperties(
      Component component, Map<String, Object> expectedProperties) {
