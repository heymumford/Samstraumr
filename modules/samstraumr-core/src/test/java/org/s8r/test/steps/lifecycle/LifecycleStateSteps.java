...
public class LifecycleStateSteps {
    ...
    public void all_operations_should_succeed() {
        ...
        for (String result : operationResults) {
            Assertions.assertFalse(result.contains("exception"),
                "Operation should not throw exception: " + result);
        }
    ...
    public void data_processing_should_be_enabled() {
        Assertions.assertTrue(isDataProcessingEnabled(), 
            "Data processing should be enabled");
    }

    @Then("monitoring should be enabled")
    public void monitoring_should_be_enabled() {
        Assertions.assertTrue(isMonitoringEnabled(), 
    ...
    public void configuration_changes_should_be_allowed() {
        Assertions.assertTrue(areConfigurationChangesAllowed(), 
            "Configuration changes should be allowed");
    }

    @Then("the component should accept incoming connections")
    public void the_component_should_accept_incoming_connections() {
        Assertions.assertTrue(areConnectionsAccepted(), 
            "Component should accept incoming connections");
    }

    @Then("data processing should be paused")
    public void data_processing_should_be_paused() {
        Assertions.assertFalse(isDataProcessingEnabled(), 
            "Data processing should be paused");
    }
    ...
    public void pending_operations_should_be_queued() {
        ...
        Assertions.assertFalse(queuedOperations.isEmpty(), 
    ...
    public void the_component_should_reject_new_data_processing_requests() {
        try {
            ...
            // Expected behavior
            Assertions.assertTrue(e.getMessage().contains("suspended") ||
                                 e.getMessage().contains("data processing"),
    ...
    public void queued_operations_should_resume() {
        ...
        Assertions.assertEquals(0, queuedOperations.size(), 
            "Queued operations should be processed");
        Assertions.assertTrue(originalQueueSize > 0, 
            "Should have had operations to process");
    }
    ...
    public void data_processing_should_continue() {
        Assertions.assertTrue(isDataProcessingEnabled(), 
            "Data processing should be enabled");
        Assertions.assertTrue(dataProcessingActive, 
            "Data processing should be active");
    }
    ...
    public void in_state_the_following_operations_should_be_available(String stateName, DataTable dataTable) {
    ...
    public void resources_should_be_allocated_during_initialization() {
        ...
        Assertions.assertTrue((int)resourceUsage.get("memory") > 0, 
            "Memory resources should be allocated");
        Assertions.assertTrue((int)resourceUsage.get("threads") > 0, 
    ...
    public void resources_should_be_fully_available_in_state(String stateName) {
        ...
        Assertions.assertTrue((int)resourceUsage.get("memory") >= 50, 
            "Memory resources should be fully available");
        Assertions.assertTrue((int)resourceUsage.get("threads") >= 2, 
            "Thread resources should be fully available");
        Assertions.assertTrue((int)resourceUsage.get("connections") >= 0, 
            "Connection resources should be fully available");
        Assertions.assertTrue((int)resourceUsage.get("timers") >= 1, 
            "Timer resources should be fully available");
    }
    ...
    public void resources_should_be_partially_released_in_state(String stateName) {
        ...
        if (stateName.equals("SUSPENDED")) {
            ...
            Assertions.assertTrue((int)resourceUsage.get("timers") < (int)resourceUsage.get("activeTimers"), 
    ...
    public void all_resources_should_be_released_in_state(String stateName) {
        ...
        Assertions.assertTrue((int)resourceUsage.get("connections") == 0, 
            "Connection resources should be released");
        Assertions.assertTrue((int)resourceUsage.get("timers") == 0, 
            "Timer resources should be released");
        Assertions.assertTrue((int)resourceUsage.get("threads") <= 1, 
    ...
    public void it_should_publish_and_receive_events() {
        ...
        Assertions.assertTrue(availableOperations.get("publishEvents"), 
            "Component should be able to publish events");
        Assertions.assertTrue(availableOperations.get("receiveEvents"), 
            "Component should be able to receive events");
    }
    ...
    public void it_should_not_publish_events_but_still_receive_them() {
        ...
        Assertions.assertEquals(State.SUSPENDED, component.getState(),
            "Component should be in SUSPENDED state");
        
        Assertions.assertFalse(availableOperations.get("publishEvents"), 
    ...
    public void it_should_publish_diagnostic_events_but_queue_normal_events() {
        ...
        try {
            ...
            Assertions.assertFalse(queuedOperations.isEmpty(), 
                "Normal events should be queued");
            ...
            Assertions.assertTrue(foundQueuedEvent, 
                "Normal events should be queued");
    ...
    public void it_should_unsubscribe_from_all_events() {
        ...
        Assertions.assertFalse(availableOperations.get("receiveEvents"), 
    ...
    public void state_constraints_should_be_maintained_during_recovery() {
        ...
        Assertions.assertEquals(State.valueOf("RECOVERING"), component.getState(),
        ...
        try {
            ...
            // Expected behavior
            Assertions.assertTrue(e.getMessage().contains("recovering"),
                "Exception should mention recovering state");
        }
    }
    ...
    private void testDataProcessing() throws Exception {
        ...
        if (component.getState() == State.SUSPENDED ||
            ...
            throw new IllegalStateException("Cannot process data when component is " + 
    ...
    private void testConfigurationChanges() throws Exception {
        ...
        if (component.getState() != State.ACTIVE && 
            component.getState() != State.MAINTENANCE) {
            throw new IllegalStateException("Cannot change configuration when component is " + 
                component.getState().name().toLowerCase());
        }
    ...
    private void testConnections() throws Exception {
        ...
        if (component.getState() != State.ACTIVE) {
            throw new IllegalStateException("Cannot establish connections when component is " + 
                component.getState().name().toLowerCase());
        }
    ...
    private boolean testOperation(String operation) throws Exception {
        switch (operation) {
            case "process_data":
                ...
                throw new IllegalStateException("Cannot process data in " + component.getState() + " state");
            ...
            case "update_config":
                ...
                throw new IllegalStateException("Cannot update config in " + component.getState() + " state");
                
            case "reset_config":
            case "run_diagnostics":
                if (component.getState() == State.MAINTENANCE || component.getState() == State.valueOf("RECOVERING")) {
                    return true;
                }
                throw new IllegalStateException("Cannot run diagnostics in " + component.getState() + " state");
                
            case "establish_connection":
                if (component.getState() == State.ACTIVE) {
                    return true;
                }
                throw new IllegalStateException("Cannot establish connection in " + component.getState() + " state");
    ...
    private void publishDiagnosticEvent() {
        ...
        if (component.getState() != State.MAINTENANCE && 
            component.getState() != State.valueOf("RECOVERING")) {
            throw new IllegalStateException("Cannot publish diagnostic events in " + 
                component.getState().name().toLowerCase() + " state");
        }
