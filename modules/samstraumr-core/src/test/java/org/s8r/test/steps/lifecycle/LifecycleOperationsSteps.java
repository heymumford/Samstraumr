...
/**
 * Step definitions focused on component operations in different lifecycle states. This class tests
 * the operational aspects of the Component lifecycle.
 */
public class LifecycleOperationsSteps {
    ...
    for (Map.Entry<String, Object> entry : operationResults.entrySet()) {
      if (entry.getValue() instanceof Boolean) {
        Assertions.assertTrue(
            (Boolean) entry.getValue(), "Operation " + entry.getKey() + " should succeed");
      }
    }

    Assertions.assertFalse(
        operationResults.containsKey("exception"), "No exceptions should occur during operations");
  }
  ...
  public void the_new_configuration_should_be_applied() {
    ...
    Assertions.assertEquals(
        "new-value1", env.getValue("advanced.setting1"), "Setting 1 should be updated");
    Assertions.assertEquals(
        "new-value2", env.getValue("advanced.setting2"), "Setting 2 should be updated");
  }
  ...
  public void data_processing_should_resume_with_new_configuration() {
    ...
    Environment env = component.getEnvironment();
    Assertions.assertEquals(
        "new-value1", env.getValue("advanced.setting1"), "Setting 1 should still be applied");
  }
  ...
  public void resources_should_be_allocated_during_initialization() {
    Assertions.assertTrue(
        component.getResourceUsage("memory") > 0, "Memory resources should be allocated");
    Assertions.assertTrue(
        component.getResourceUsage("threads") > 0, "Thread resources should be allocated");
  }
  ...
  public void resources_should_be_fully_available_in_state(String stateName) {
    ...
    Assertions.assertTrue(
        ComponentTestUtil.verifyResourceAvailability(component, "memory", 40),
        "Memory resources should be fully available");
    Assertions.assertTrue(
        ComponentTestUtil.verifyResourceAvailability(component, "threads", 2),
  ...
  public void resources_should_be_partially_released_in_state(String stateName) {
    ...
    Assertions.assertTrue(
        currentConnections < initialConnections, "Connection resources should be reduced");
    Assertions.assertTrue(currentTimers < initialTimers, "Timer resources should be reduced");
  }
  ...
  public void it_should_publish_and_receive_events() {
    ...
    Assertions.assertTrue(
        eventListener.getEventCount() > 0, "Event listener should have received the event");
  }
  ...
  public void it_should_not_publish_events_but_still_receive_them() {
    ...
    senderComponent.addEventListener(
        (c, type, data) -> {
  ...
  public void it_should_publish_diagnostic_events_but_queue_normal_events() {
    ...
    Assertions.assertTrue(
        diagnosticPublished, "Diagnostic event should be published in MAINTENANCE state");
    Assertions.assertTrue(
        eventListener.getEventCount() > 0,
  ...
  public void it_should_unsubscribe_from_all_events() {
    ...
    Assertions.assertTrue(
        eventListener.isTerminated(), "Event listener should have been terminated");
  }
  ...
  public void it_should_not_publish_any_events() {
    ...
    Assertions.assertEquals(
        0, eventListener.getEventCount(), "No events should be received after termination");
  }
  ...
  public void state_constraints_should_be_maintained_during_recovery() {
    Assertions.assertEquals(
        State.RECOVERING, component.getState(), "Component should be in RECOVERING state");
  ...
  public void normal_operations_should_resume() {
    ...
    Assertions.assertEquals(
        State.ACTIVE, component.getState(), "Component should be in ACTIVE state after recovery");
