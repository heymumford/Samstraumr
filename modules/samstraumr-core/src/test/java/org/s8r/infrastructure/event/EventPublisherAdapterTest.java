 ...
 *
 * <p>These tests verify that the EventPublisherAdapter correctly implements the EventPublisherPort
 * interface and properly delegates to the EventDispatcher.
 */
@UnitTest
public class EventPublisherAdapterTest {
  ...
  void setUp() {
    ...
    eventPublisherAdapter =
        new EventPublisherAdapter(mockEventDispatcher, mockComponentRepository, mockLogger);
  }
  ...
  void publishEvent_shouldDispatchSingleEvent() {
    // Arrange
    ComponentId componentId = ComponentId.create("test-component-id");
    DomainEvent testEvent = new ComponentCreatedEvent(componentId, "TestComponent");
  ...
  void publishEvents_shouldDispatchMultipleEvents() {
    // Arrange
    ComponentId componentId1 = ComponentId.create("test-component-1");
    ComponentId componentId2 = ComponentId.create("test-component-2");
    DomainEvent event1 = new ComponentCreatedEvent(componentId1, "TestComponent1");
    DomainEvent event2 =
        new ComponentStateChangedEvent(
            componentId2, LifecycleState.CONCEPTION, LifecycleState.READY, "Test transition");
  ...
  void publishPendingEvents_shouldPublishAndClearEvents() {
    ...
    ComponentId eventComponentId = ComponentId.create("test-component-id");
    componentEvents.add(new ComponentCreatedEvent(eventComponentId, "TestComponent"));
    componentEvents.add(
        new ComponentStateChangedEvent(
            eventComponentId, LifecycleState.CONCEPTION, LifecycleState.READY, "Test transition"));
