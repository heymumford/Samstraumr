...
public class ManualCompositeConnectionTest {
  ...
  public static void main(String[] args) {
    ...
    // Add A -> B connection
    connections.add(
        new ComponentConnection(componentA, componentB, ConnectionType.DATA_FLOW, "Test A->B"));

    try {
      CompositeConnectionValidator.validateConnection(
          compositeId, componentB, componentC, ConnectionType.DATA_FLOW, connections, id -> true);

      System.out.println("✓ Non-cycle connection accepted (B->C)");

      // Add B -> C connection
      connections.add(
          new ComponentConnection(componentB, componentC, ConnectionType.DATA_FLOW, "Test B->C"));
    } catch (Exception e) {
      System.out.println("✗ Unexpected exception: " + e.getMessage());
    }
    ...
    try {
      CompositeConnectionValidator.validateConnection(
          compositeId, componentC, componentA, ConnectionType.DATA_FLOW, connections, id -> true);
    ...
    peerConnections.add(
        new ComponentConnection(componentA, componentB, ConnectionType.PEER, "A->B peer"));

    try {
      CompositeConnectionValidator.validateConnection(
          compositeId, componentB, componentA, ConnectionType.PEER, peerConnections, id -> true);
