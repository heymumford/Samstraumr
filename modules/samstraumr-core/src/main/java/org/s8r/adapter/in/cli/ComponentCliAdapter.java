...
public class ComponentCliAdapter {
  ...
  public ComponentCliAdapter() {
    ...
    LoggerFactory loggerFactory =
        org.s8r.infrastructure.config.DependencyContainer.getInstance().get(LoggerFactory.class);
    ...
    EventPublisherPort eventPublisher =
        new EventPublisherAdapter(eventDispatcher, repository, logger);
  ...
  private void listAllComponents() {
    try {
      ...
      for (ComponentPort c : components) {
        String type =
            c instanceof org.s8r.domain.component.port.CompositeComponentPort
                ? "Composite"
                : "Component";
        System.out.printf(
            "ID: %s | Type: %s | State: %s%n", c.getId().getShortId(), type, c.getLifecycleState());
      }
    } catch (Exception e) {
