...
/**
 * A concrete implementation of LegacyIdentityConverter specifically for Tube identities. This class
 * provides direct conversion between TubeIdentity and ComponentId without using reflection.
 *
 * <p>This is more efficient than the reflection-based approach when we know we're working with Tube
 * identities specifically, and serves as the primary migration path for existing code.
 */
public class TubeLegacyIdentityConverter implements LegacyIdentityConverter {
  ...
  public Object createLegacyChildIdentity(
    ...
    if (!(parentLegacyIdentity instanceof TubeIdentity)) {
      throw new IllegalArgumentException(
          "Expected TubeIdentity object, got: "
    ...
    logger.debug(
        "Creating legacy child identity with reason: {} and parent: {}",
        reason,
