...
/**
 * A concrete implementation of LegacyEnvironmentConverter specifically for Tube environments. This
 * class provides direct conversion between Environment objects without using reflection.
 *
 * <p>This is more efficient than the reflection-based approach when we know we're working with Tube
 * environments specifically, and serves as the primary migration path for existing code.
 */
public class TubeLegacyEnvironmentConverter implements LegacyEnvironmentConverter {
  ...
  public Object createLegacyEnvironment(Map<String, String> parameters) {
    logger.debug(
        "Creating legacy environment with {} parameters",
  ...
  public Map<String, String> extractParametersFromLegacyEnvironment(Object legacyEnvironment) {
    if (!(legacyEnvironment instanceof Environment)) {
      throw new IllegalArgumentException(
          "Expected Environment object, got: "
  ...
  /**
   * Creates a new environment object with the given name and parameters. This is a helper method
   * specific to Tube environments, providing a simpler API than the generic createLegacyEnvironment
   * method.
   *
  ...
  public Environment createNamedEnvironment(String name, Map<String, String> parameters) {
    logger.debug(
        "Creating named environment: {} with {} parameters",
        name,
        parameters != null ? parameters.size() : 0);
   ...
   */
  public org.s8r.component.core.Environment fromLegacyEnvironmentToCore(
