 ...
 *
 * <p>This adapter bridges between the domain layer's port interface and the actual MachineFactory
 * implementation, allowing the application core to remain independent of specific machine creation
 * details.
 */
public class MachineFactoryAdapter implements MachineFactoryPort {
  ...
  public MachinePort createMachine(ComponentId id, MachineType type) {
    ...
    Machine machine = MachineFactory.createMachine(id, type, name, description, "1.0.0");
    return MachineAdapter.createMachinePortFromDomain(machine);
  }

  /**
   * Creates a machine with the given name and type string. This is a convenience method that's not
  ...
  public MachinePort createMachine(String name, String type) {
    ...
    try {
      ...
      Machine machine =
          MachineFactory.createMachine(MachineType.DATA_PROCESSOR, name, name, "1.0.0");
      return MachineAdapter.createMachinePortFromDomain(machine);
    }
  }
  ...
  public MachinePort cloneMachine(MachinePort sourceMachine, ComponentId newId) {
    logger.debug(
        "Cloning machine {} to new ID {}", sourceMachine.getId().getShortId(), newId.getShortId());
