...
/**
 * Implementation of the Tube concept which can choose between immortality and reproduction based on
...
public class Tube {
  ...
  public void addReproductionOutcome(ReproductionOutcome outcome) {
    ...
    LOGGER.info(
        "Added reproduction outcome for attempt #"
            + outcome.getAttempt()
            + " with success rate: "
            + outcome.getSuccessRate());
  }
  ...
  public SurvivalStrategy evaluateSurvivalStrategy(TubeEnvironment environment) {
    ...
    // Set strategy mode
    strategyMode =
        StrategyMode.forStrategy(strategy, populationDensity, hostilityLevel, favorability);
   ...
   */
  private void logStrategyDecision(
      SurvivalStrategy strategy, StrategyMode mode, TubeEnvironment environment) {
    LOGGER.info(
        "Tube evaluated survival strategy based on environment: " + environment.getSummary());
    LOGGER.info(
        "Selected strategy: "
            + strategy
            + ", mode: "
            + mode
            + ", confidence: "
            + String.format("%.2f", decisionConfidence));
  ...
  public void initiateReproduction() {
    if (survivalStrategy != SurvivalStrategy.REPRODUCTION
      ...
      LOGGER.warning(
          "Attempted to initiate reproduction with incompatible strategy: " + survivalStrategy);
      return;
    }
    ...
    List<KnowledgeType> sortedKnowledge =
  ...
  public void interruptReproduction(double completionPercentage) {
    LOGGER.warning(
        "Reproduction process interrupted at "
            + String.format("%.2f", completionPercentage * 100)
            + "% completion");
   ...
   */
  public void adaptReproductionStrategy(
      List<ReproductionOutcome> history, TubeEnvironment environment) {
    LOGGER.info(
        "Adapting reproduction strategy based on "
            + history.size()
    ...
    for (ReproductionOutcome outcome : history) {
      successRatesByEnvironment.compute(
          ...
          (k, v) -> (v == null) ? outcome.getSuccessRate() : (v + outcome.getSuccessRate()) / 2);
    }
    ...
    LOGGER.info(
        "Adapted strategy for current environment (" + currentEnvironment + "): " + bestStrategy);
   ...
   */
  private String findBestStrategyForEnvironment(
      List<ReproductionOutcome> history, String environmentType) {
