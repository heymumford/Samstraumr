 ...
 *
 * <p>This event follows the standardized naming convention without the "Event" suffix, which is
 * recommended for domain events in Clean Architecture. It uses the Component ID to uniquely
 * identify the component that was created.
 *
 * <p>This implementation is designed to work seamlessly with both direct domain entities and port
 * interfaces through the adapter layer, supporting the Dependency Inversion Principle of Clean
 * Architecture.
 */
public class ComponentCreated extends DomainEvent {
   ...
   * @param componentType The type of the component (e.g., "Component", "CompositeComponent",
   *     "Machine")
   */
  ...
  public ComponentCreated(
      ComponentId componentId, LifecycleState initialState, String componentType) {
    super(componentId.getIdString());
    this.componentId = componentId;
    this.componentType = componentType;
  ...
  public String toString() {
    return String.format(
        "ComponentCreated[componentId=%s, initialState=%s, componentType=%s]",
