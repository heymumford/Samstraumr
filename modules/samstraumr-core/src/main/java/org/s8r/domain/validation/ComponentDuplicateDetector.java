...
/**
 * Component duplicate detector that analyzes components for potential duplicates based on property
 * analysis.
 *
 * <p>This class implements sophisticated detection of duplicate components beyond simple ID
 * comparison. It uses pattern matching, property analysis, and similarity scoring to identify
...
public class ComponentDuplicateDetector {
  ...
  private static final Set<String> DEFAULT_PROPERTIES =
      new HashSet<>(
          Arrays.asList("name", "type", "category", "purpose", "function", "description"));
   ...
   */
  public ComponentDuplicateDetector(
      ComponentRepository componentRepository, LoggerPort logger, double defaultThreshold) {
  ...
  /**
   * Creates a new ComponentDuplicateDetector with the specified repository and a default threshold
   * of 0.8.
   *
   * @param componentRepository The repository to use for component lookup
   * @param logger The logger to use for logging duplicate detection events
  ...
  private void initializeDefaultPropertyMatchers() {
    ...
    addPropertyMatcher(
        "description", (value1, value2) -> compareStringsSimilarity(value1, value2) * 0.7);
  }
  ...
  /**
   * Sets whether strict mode is enabled. In strict mode, the detector will throw exceptions for
   * potential duplicates rather than just reporting them.
   *
   ...
   * @return this detector instance for method chaining
   */
  public ComponentDuplicateDetector addPropertyMatcher(
      String propertyName, PropertyMatcher matcher) {
  ...
  /**
   * Sets the properties to use for comparison during duplicate detection. This replaces any
   * existing property matchers.
   *
   ...
   * @throws PropertyBasedDuplicateComponentException if a duplicate is found and strict mode is
   *     enabled
  ...
  /**
   * Checks a component for potential duplicates among all components in the repository, using a
   * specific similarity threshold.
   *
   * @param component The component to check for duplicates
   * @param threshold The similarity threshold (0-1) above which components are considered
   *     duplicates
   * @throws PropertyBasedDuplicateComponentException if a duplicate is found and strict mode is
   *     enabled
   * @return A list of potential duplicate components, or an empty list if none are found
  ...
  public List<ComponentPort> checkForDuplicates(ComponentPort component, double threshold) {
    ...
    logger.debug(
        "Checking component {} for duplicates with threshold {}",
        targetId.getIdString(),
        threshold);
    ...
    for (ComponentPort existingComponent : allComponents) {
      ...
      if (result.getSimilarityScore() >= threshold) {
        ...
        logger.debug(
            "Potential duplicate found: {} with similarity score {}",
            existingComponent.getId().getIdString(),
  ...
  public DuplicateAnalysisResult analyzePotentialDuplicate(
    ...
    for (String property : propertiesToCompare) {
      ...
      PropertyMatcher matcher = propertyMatchers.getOrDefault(property, this::compareGenericValues);
  ...
  private String generateDuplicateReason(
      Set<String> matchingProperties, Map<String, Double> propertyScores, double overallScore) {
    ...
    List<String> topMatches =
        matchingProperties.stream()
            .sorted(
                (p1, p2) ->
                    Double.compare(
                        propertyScores.getOrDefault(p2, 0.0), propertyScores.getOrDefault(p1, 0.0)))
            .limit(3)
            .collect(Collectors.toList());

    return String.format(
        "Matching properties include: %s. Overall similarity score: %.2f",
  ...
  /**
   * Functional interface for property matchers that calculate similarity between two property
   * values.
   */
  @FunctionalInterface
