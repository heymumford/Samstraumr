...
/**
 * Validator for composite connections to prevent cycles and ensure valid component references.
 * This utility class provides validation for connections between components in a composite structure.
 */
public class CompositeConnectionValidator {
    ...
    private static void validateNoCycles(
        ...
        if (!cyclePath.isEmpty()) {
            ...
            throw new ConnectionCycleException(
                    String.format("Connection from %s to %s would create a cycle: %s", 
                            sourceId.getShortId(), 
                            targetId.getShortId(),
                            cyclePath.stream().map(ComponentId::getShortId).reduce((a, b) -> a + " -> " + b).orElse("")),
                    sourceId,
                    targetId,
                    cyclePath);
        }
     ...
     * @return A map representing the connection graph, where keys are source IDs and values are sets of target IDs
     */
    private static Map<ComponentId, Set<ComponentId>> buildConnectionGraph(List<ComponentConnection> connections) {
     ...
     */
    private static void addToGraph(Map<ComponentId, Set<ComponentId>> graph, ComponentId from, ComponentId to) {
     ...
     */
    private static List<ComponentId> findCycle(Map<ComponentId, Set<ComponentId>> graph, ComponentId start, ComponentId end) {
    ...
    /**
     * Checks if a connection type is directed (forms a hierarchy).
     * Some connection types like PEER or SIBLING don't form hierarchical relationships.
     *
