 ...
 *
 * <p>This adapter provides a simple security implementation for development and testing purposes.
 * It stores all security-related information in memory and will lose data when the application is
 * restarted.
 *
 * <p>For production use, this should be replaced with an adapter that connects to a proper security
 * system.
 */
public class InMemorySecurityAdapter implements SecurityPort {
  ...
  private final Map<String, Map<String, Set<Permission>>> resourcePermissions =
  ...
  /**
   * Simulates hashing a password. In a real implementation, this would use a secure password
   * hashing algorithm like BCrypt, PBKDF2, or Argon2.
   *
  ...
  public SecurityResult authenticate(String username, String password) {
    ...
    if (user == null) {
      logSecurityEvent(
          ...
          Map.of("username", username, "reason", "User not found"));
    ...
    if (!verifyPassword(password, user.getPasswordHash())) {
      logSecurityEvent(
          SecurityEventType.LOGIN_FAILURE,
          Map.of("username", username, "reason", "Invalid password"));
      return SecurityResult.failure("Authentication failed", "Invalid username or password");
    }
    ...
    logSecurityEvent(
        SecurityEventType.LOGIN_SUCCESS, Map.of("userId", user.getUserId(), "username", username));
  ...
  public SecurityResult authenticateWithToken(String token) {
    ...
    if (!tokenEntry.isValid()) {
      logSecurityEvent(
          ...
          Map.of(
              ...
              "failure",
              "reason",
              tokenEntry.isRevoked() ? "Token revoked" : "Token expired"));
    ...
    logSecurityEvent(
        SecurityEventType.TOKEN_VALIDATED,
        Map.of("token", token, "result", "success", "userId", user.getUserId()));
  ...
  public SecurityResult generateToken(Duration validity) {
    ...
    TokenEntry tokenEntry =
        new TokenEntry(token, authContext.getUserId(), Instant.now(), tokenValidity);
    ...
    return SecurityResult.success(
        ...
        Map.of(
            "token", token, "expiresAt", tokenEntry.getCreatedAt().plus(tokenValidity).toString()));
  }
  ...
  public SecurityResult validateToken(String token) {
    ...
    logSecurityEvent(
        SecurityEventType.TOKEN_VALIDATED,
        Map.of("token", token, "result", "success", "userId", tokenEntry.getUserId()));
  ...
  public SecurityResult revokeToken(String token) {
    ...
    logSecurityEvent(
        ...
        Map.of("token", token, "userId", tokenEntry.getUserId(), "reason", "Manually revoked"));
  ...
  public SecurityResult checkComponentAccess(
    ...
    if (componentId == null || resourceId == null || operationType == null) {
      return SecurityResult.failure(
          "Access check failed", "Component ID, resource ID, and operation type cannot be null");
    }
  ...
  public SecurityResult registerUser(String username, String password, Set<String> roles) {
    if (username == null || password == null) {
      return SecurityResult.failure(
          "User registration failed", "Username and password cannot be null");
    }

    // Check if username already exists
    boolean userExists = users.values().stream().anyMatch(u -> u.getUsername().equals(username));
    ...
    UserEntry newUser =
        new UserEntry(userId, username, passwordHash, userRoles, permissions, new HashMap<>());
  ...
  private Set<String> derivePermissionsFromRoles(Set<String> roles) {
    ...
    if (roles.contains(ADMIN_ROLE)) {
      permissions.addAll(
          Set.of("READ", "WRITE", "DELETE", "ADMIN", "CONFIGURE", "MANAGE_USERS", "VIEW_LOGS"));
    } else if (roles.contains(USER_ROLE)) {
      permissions.addAll(Set.of("READ", "WRITE"));
    }
  ...
  private void setupDefaultComponentAccess() {
    ...
    // This is a simplified example. In a real system, this would be loaded from a configuration
    // file
    // or database.
  ...
  public boolean hasPermission(SecurityContext context, String resource, Permission permission) {
    ...
    Map<String, Set<Permission>> contextPermissions =
        resourcePermissions.getOrDefault(contextId, new HashMap<>());
  ...
  public void grantPermission(SecurityContext context, String resource, Permission permission) {
    ...
    String contextId = context.getIdentifier();
    Map<String, Set<Permission>> contextPermissions =
        resourcePermissions.computeIfAbsent(contextId, k -> new ConcurrentHashMap<>());

    Set<Permission> resourcePerms =
        contextPermissions.computeIfAbsent(resource, k -> new HashSet<>());
