 ...
 *
 * <p>This class provides access to adapters that convert between legacy and new object
 * representations, while maintaining clean architecture boundaries.
 *
 * <p>This implementation uses reflective adapters to avoid direct dependencies on legacy code,
 * following Clean Architecture principles. The adapters use reflection to interact with legacy
 * classes, removing compile-time dependencies.
 */
    ...
    if (coreEnvConverter == null) {
      try {
        ...
        try {
          ...
          coreEnvConverter =
              (LegacyEnvironmentConverter) converterClass.getDeclaredConstructor().newInstance();
    ...
    if (coreComponentAdapter == null) {
      try {
        ...
        coreComponentAdapter =
            new org.s8r.adapter.LegacyComponentAdapter(
