...
public class DataFlowEventHandler implements DataFlowEventPort {
  ...
  public void publishData(ComponentId componentId, String channel, Map<String, Object> data) {
    if (componentId == null || channel == null || data == null) {
      logger.warn(
          "Cannot publish data: componentId={}, channel={}, data={}", componentId, channel, data);
      return;
    }

    logger.debug(
        "Publishing data from component [{}] to channel [{}]", componentId.getShortId(), channel);

    // Create a ComponentDataEvent
    ComponentDataEvent event = new ComponentDataEvent(componentId, channel, data);
