 ...
 *
 * <p>This adapter connects a component to the data flow event infrastructure, simplifying the
 * subscription and publication of data events without the component directly depending on the
 * infrastructure implementation.
 *
 * <p>This follows the adapter pattern of Clean Architecture, adapting between the domain layer's
 * port interface and the infrastructure layer's implementation.
 */
public class DataFlowComponentAdapter implements DataFlowComponentPort {
  ...
  public DataFlowComponentAdapter(
      ComponentId componentId, DataFlowEventPort dataFlowEventPort, LoggerPort logger) {
  ...
  public boolean subscribe(String channel, Consumer<ComponentDataEvent> handler) {
    ...
    try {
      ...
      logger.debug("Component {} subscribed to channel: {}", componentId.getIdString(), channel);
      return true;
    } catch (Exception e) {
      logger.error(
          "Error subscribing component {} to channel {}: {}",
          componentId.getIdString(),
          channel,
          e.getMessage(),
          e);
      return false;
  ...
  public boolean unsubscribe(String channel) {
    ...
    try {
      ...
      logger.debug(
          "Component {} unsubscribed from channel: {}", componentId.getIdString(), channel);
      return true;
    } catch (Exception e) {
      logger.error(
          "Error unsubscribing component {} from channel {}: {}",
          componentId.getIdString(),
          channel,
          e.getMessage(),
          e);
  ...
  public boolean unsubscribeAll() {
    try {
      ...
      subscriptions.clear();
      logger.debug(
          "Component {} unsubscribed from all channels ({})", componentId.getIdString(), count);
      return true;
    } catch (Exception e) {
      logger.error(
          "Error unsubscribing component {} from all channels: {}",
          componentId.getIdString(),
          e.getMessage(),
          e);
      return false;
  ...
  public boolean publish(String channel, Map<String, Object> data) {
    ...
    try {
      ...
      logger.debug(
          "Component {} published data to channel: {}", componentId.getIdString(), channel);
      return true;
    } catch (Exception e) {
      logger.error(
          "Error publishing data from component {} to channel {}: {}",
          componentId.getIdString(),
          channel,
          e.getMessage(),
          e);
