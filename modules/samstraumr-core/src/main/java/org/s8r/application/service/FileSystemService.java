 ...
 *
 * <p>This service uses the FileSystemPort to interact with the file system infrastructure. It
 * provides a simplified interface for common file operations and adds additional business logic as
 * needed.
 */
public class FileSystemService {
  ...
  public void initialize() {
    ...
    if (!result.isSuccessful()) {
      logger.error(
          "Failed to initialize file system service: {}",
  ...
  public boolean createDirectory(String path) {
    ...
    if (!result.isSuccessful()) {
      logger.warn(
          "Failed to create directory {}: {}", path, result.getReason().orElse("Unknown reason"));
    }
  ...
  public boolean writeFile(String path, String content, Charset charset, boolean append) {
    Charset fileCharset = (charset != null) ? charset : StandardCharsets.UTF_8;
    FileSystemPort.FileResult result =
        fileSystemPort.writeString(path, content, fileCharset, append);
    if (!result.isSuccessful()) {
      logger.warn(
          "Failed to write to file {}: {}", path, result.getReason().orElse("Unknown reason"));
    }
    return result.isSuccessful();
  }
   ...
   *     directory
   * @return An Optional containing the path of the created file, or empty if the file could not be
   *     created
   */
  public Optional<String> createTempFile(String prefix, String suffix, String directory) {
    ...
    if (result.isSuccessful()) {
      ...
      logger.warn(
          "Failed to create temporary file: {}", result.getReason().orElse("Unknown reason"));
      return Optional.empty();
    }
  }
   ...
   *     directory
   * @return An Optional containing the path of the created directory, or empty if the directory
  ...
  public Optional<String> createTempDirectory(String prefix, String directory) {
    ...
    if (result.isSuccessful()) {
      ...
      logger.warn(
          "Failed to create temporary directory: {}", result.getReason().orElse("Unknown reason"));
      return Optional.empty();
    }
  }
   ...
   * @return An Optional containing the list of file information, or empty if the directory could
   *     not be listed
   */
   ...
   * @return An Optional containing the list of matching file information, or empty if the search
   *     failed
   */
  public Optional<List<FileSystemPort.FileInfo>> searchFiles(
      String directory, String pattern, boolean recursive) {
    FileSystemPort.SearchMode searchMode =
        recursive
  ...
  public void shutdown() {
    ...
    if (!result.isSuccessful()) {
      logger.error(
          "Failed to shut down file system service: {}",
          result.getReason().orElse("Unknown reason"));
    }
  }
}
