 ...
 *
 * <p>This service uses the ValidationPort to provide data validation services to the application.
 * It provides high-level validation operations while relying on the port interface for the actual
 * validation logic.
 */
public class ValidationService {
  ...
  public ValidationResult validateMachineCreation(
    ...
    if (!typeResult.isValid() || !nameResult.isValid() || !versionResult.isValid()) {
      return ValidationResult.invalid(
          List.of("Machine creation requires valid type, name, and version"));
    }
  ...
  public ValidationResult validateAllowedValues(
    ...
    return validationPort.validateAllowedValues(
        fieldName, value, allowedValues.stream().map(v -> (Object) v).toList());
  }
