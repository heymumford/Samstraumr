 ...
 *
 * <p>This service uses the TaskExecutionPort to interact with the task execution infrastructure. It
 * provides a simplified interface for common task operations and adds additional business logic as
 * needed.
 */
public class TaskExecutionService {
  ...
  public boolean initialize() {
    ...
    if (!result.isSuccessful()) {
      logger.error(
          "Failed to initialize task execution service: {}",
   ...
   * @return The task ID, or empty if the task could not be scheduled
   */
  public Optional<String> scheduleTask(
      Runnable runnable, Instant executionTime, TaskOptions options) {
    if (executionTime.isBefore(Instant.now())) {
      logger.warn("Cannot schedule task in the past: {}", executionTime);
      return Optional.empty();
    }

    TaskExecutionPort.TaskResult result =
        taskExecutionPort.scheduleTask(runnable, executionTime, options);
    if (result.isSuccessful()) {
      return result.getTaskId();
    } else {
      logger.warn("Failed to schedule task: {}", result.getReason().orElse("Unknown reason"));
   ...
   * @return The task ID, or empty if the task could not be scheduled
   */
  public <T> Optional<String> scheduleTask(
    ...
    TaskExecutionPort.TaskResult result =
        taskExecutionPort.scheduleTask(callable, executionTime, options);
    if (result.isSuccessful()) {
      return result.getTaskId();
    } else {
      logger.warn("Failed to schedule task: {}", result.getReason().orElse("Unknown reason"));
  ...
  public Optional<Task> getTask(String taskId) {
    ...
    if (result.isSuccessful()) {
      ...
      if (result.getReason().isPresent() && !result.getReason().get().contains("not found")) {
        logger.warn(
            "Failed to get task {}: {}", taskId, result.getReason().orElse("Unknown reason"));
      }
      return Optional.empty();
    }
  }
  ...
  public boolean cancelTask(String taskId, boolean mayInterruptIfRunning) {
    TaskExecutionPort.TaskResult result =
        taskExecutionPort.cancelTask(taskId, mayInterruptIfRunning);
    if (!result.isSuccessful()) {
      logger.warn(
          "Failed to cancel task {}: {}", taskId, result.getReason().orElse("Unknown reason"));
    }
  ...
  public boolean onTaskCompletion(String taskId, Consumer<Task> listener) {
    TaskExecutionPort.TaskResult result =
        taskExecutionPort.registerCompletionListener(taskId, listener);
    if (!result.isSuccessful()) {
      logger.warn(
          "Failed to register completion listener for task {}: {}",
          taskId,
          result.getReason().orElse("Unknown reason"));
    }
    return result.isSuccessful();
   ...
   * @param <T> The type of the task result
   * @param callable The callable to execute
   * @return A CompletableFuture that will complete with the task result, or complete exceptionally
   *     if the task fails
   */
        ...
        task -> {
          if (task.getStatus() == TaskStatus.COMPLETED) {
            ...
            future.completeExceptionally(
                new RuntimeException(task.getErrorMessage().orElse("Task failed")));
  ...
  public TaskOptions createTaskWithTimeout(String name, String description, Duration timeout) {
    return createOptionsBuilder().name(name).description(description).timeout(timeout).build();
  }
  ...
  public Map<String, Object> getStatistics() {
    ...
    if (result.isSuccessful()) {
      ...
      logger.warn(
          "Failed to get task execution statistics: {}",
          result.getReason().orElse("Unknown reason"));
      return Map.of();
    }
  }
  ...
  public boolean shutdown() {
    ...
    if (!result.isSuccessful()) {
      logger.error(
          "Failed to shut down task execution service: {}",
          result.getReason().orElse("Unknown reason"));
      return false;
    }
    return true;
