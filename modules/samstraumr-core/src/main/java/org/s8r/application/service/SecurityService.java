 ...
 *
 * <p>This service uses the SecurityPort to interact with the security infrastructure. It provides a
 * simplified interface for common security operations and adds additional business logic as needed.
 */
public class SecurityService {
  ...
  public void initialize() {
    ...
    if (!result.isSuccessful()) {
      logger.error(
          "Failed to initialize security service: " + result.getReason().orElse("Unknown reason"));
    }
  }
  ...
  public Optional<String> getCurrentUsername() {
    return securityPort
        ...
        .map(SecurityPort.AuthenticationContext::getUsername);
  }
  ...
  public boolean checkAccess(String componentId, String resourceId, String operationType) {
    SecurityPort.SecurityResult result =
        securityPort.checkComponentAccess(componentId, resourceId, operationType);
  ...
  public boolean logSecurityEvent(
      SecurityPort.SecurityEventType eventType, Map<String, Object> details) {
  ...
  public void shutdown() {
    ...
    if (!result.isSuccessful()) {
      logger.error(
          "Failed to shut down security service: " + result.getReason().orElse("Unknown reason"));
    }
  }
}
