...
/**
 * A Bundle represents a collection of connected Tubes that form a processing pipeline.
 * Bundles allow data to flow between tubes in a directed manner.
 */
public class Bundle {
    ...
    public Bundle createTube(String name, String reason) {
        Tube tube = new Tube(reason, environment);
    ...
    public Bundle connect(String sourceName, String targetName) {
        ...
        connections.computeIfAbsent(sourceName, k -> new ArrayList<>())
                  .add(targetName);
    ...
    private <T> Optional<T> processInternal(String tubeName, BundleData<T> bundleData) {
        ...
        try {
            ...
            for (String downstreamTube : downstreamTubes) {
                ...
                Optional<T> result = processInternal(downstreamTube, new BundleData<>(bundleData.getData()));
                if (result.isPresent()) {
