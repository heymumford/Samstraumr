 ...
 * This class provides methods for reporting different types of migration issues
 * and integrates with the MigrationFeedback system.
 */
public class MigrationIssueLogger {
    ...
    public MigrationIssue reportTypeMismatch(String property, String legacyType, String s8rType) {
        String message = String.format("Type mismatch: %s expected %s but got %s", 
                property, s8rType, legacyType);
        
        if (reportToConsole) {
            logger.warn(message);
        }
        
        MigrationIssue issue = createIssue(IssueType.TYPE_MISMATCH, Severity.WARNING)
                .property(property)
                .message(message)
                .legacyValue(legacyType)
                .s8rValue(s8rType)
                .recommendation(String.format("Convert %s to %s before assigning to %s", 
                        legacyType, s8rType, property))
                .build();
    ...
    public MigrationIssue reportPropertyNotFound(String property) {
        ...
        MigrationIssue issue = createIssue(IssueType.MISSING_PROPERTY, Severity.WARNING)
                .property(property)
                .message(message)
                .recommendation(String.format("Check if property %s should be mapped to a different name", property))
                .build();
     ...
     * @return The created MigrationIssue
     */
    public MigrationIssue reportStateTransitionIssue(String fromState, String toState, String reason) {
        String message = String.format("Invalid state transition from %s to %s: %s", 
        ...
        MigrationIssue issue = createIssue(IssueType.STATE_TRANSITION, Severity.WARNING)
                .message(message)
                .legacyValue(fromState)
                .s8rValue(toState)
                .recommendation(String.format("Ensure proper state transition path from %s to %s", 
                        fromState, toState))
                .build();
    ...
    public MigrationIssue reportReflectionError(String target, Exception exception) {
        String message = String.format("Reflection error accessing %s: %s", 
        ...
        MigrationIssue issue = createIssue(IssueType.REFLECTION_ERROR, Severity.ERROR)
                .message(message)
                .property(target)
     ...
     * @return The created MigrationIssue
     */
    public MigrationIssue reportMethodMappingIssue(String legacyMethod, String s8rMethod, String reason) {
        String message = String.format("Method mapping issue between %s and %s: %s", 
                legacyMethod, s8rMethod, reason);
        
        if (reportToConsole) {
            logger.warn(message);
        }
        
        MigrationIssue issue = createIssue(IssueType.METHOD_MAPPING, Severity.WARNING)
                .message(message)
    ...
    public MigrationIssue reportStructuralDifference(String legacyStructure, String s8rStructure) {
        ...
        MigrationIssue issue = createIssue(IssueType.STRUCTURAL_DIFFERENCE, Severity.INFO)
                .message(message)
    ...
    public MigrationIssue reportCustomIssue(IssueType type, Severity severity, String message) {
        ...
        MigrationIssue issue = createIssue(type, severity)
                .message(message)
                .build();
