 ...
 * <p>
 * This class provides the entry point for visualizing an S8r model structure
 * from the command line. It follows the Adapter pattern from Clean Architecture,
 * connecting external interfaces (CLI) to the application layer.
 */
public class ListModelCommand {
    ...
    public static void main(String[] args) {
        ...
        MachineService machineService = ServiceLocator.getServiceFactory()
                .getService(MachineService.class);
        ComponentService componentService = ServiceLocator.getServiceFactory()
                .getService(ComponentService.class);
        ...
        for (Map.Entry<String, CompositeComponentDto> entry : composites.entrySet()) {
            ...
            if (!components.isEmpty()) {
                ...
                for (Map.Entry<String, ComponentDto> compEntry : components.entrySet()) {
                    System.out.println("      - " + compEntry.getKey() + 
                            " (" + compEntry.getValue().getId() + ")");
                }
            }
        }
