...
/**
 * Factory for creating reflection-based adapters that work with legacy code without direct dependencies.
 * <p>
...
public class ReflectiveAdapterFactory implements LegacyAdapterResolver {
    ...
    public LegacyEnvironmentConverter getTubeEnvironmentConverter() {
        if (tubeEnvironmentConverter == null) {
            try {
                tubeEnvironmentConverter = new ReflectiveEnvironmentConverter(TUBE_ENVIRONMENT_CLASS, logger);
    ...
    public LegacyEnvironmentConverter getCoreEnvironmentConverter() {
        if (coreEnvironmentConverter == null) {
            try {
                coreEnvironmentConverter = new ReflectiveEnvironmentConverter(CORE_ENVIRONMENT_CLASS, logger);
    ...
    public LegacyIdentityConverter getTubeIdentityConverter() {
        if (tubeIdentityConverter == null) {
            try {
                ...
                tubeIdentityConverter = new ReflectiveIdentityConverter(
                    TUBE_IDENTITY_CLASS, envConverter, logger);
    ...
    public LegacyIdentityConverter getCoreIdentityConverter() {
        if (coreIdentityConverter == null) {
            try {
                ...
                coreIdentityConverter = new ReflectiveIdentityConverter(
                    CORE_IDENTITY_CLASS, envConverter, logger);
