 ...
 * 
 * <p>This factory provides methods to create different types of preconfigured machines
 * and a registry to track all created machines.
 */
public class MachineFactory {
     ...
     */
    public static Machine createDataProcessor(String name, String description, Environment environment) {
        String machineId = generateMachineId("DP");
        Machine machine = Machine.create(machineId, MachineType.DATA_PROCESSOR, name, description, "1.0.0", environment);
        MACHINE_REGISTRY.put(machineId, machine);
        
        // Create and configure standard data processor components
        Composite inputHandler = CompositeFactory.createComposite("input-handler", CompositeType.OBSERVER, environment);
        Composite transformer = CompositeFactory.createComposite("data-transformer", CompositeType.TRANSFORMER, environment);
        Composite outputHandler = CompositeFactory.createComposite("output-handler", CompositeType.OBSERVER, environment);
    ...
    public static Machine createAnalytics(String name, String description, Environment environment) {
        ...
        Machine machine = Machine.create(machineId, MachineType.ANALYTICS, name, description, "1.0.0", environment);
        MACHINE_REGISTRY.put(machineId, machine);
        
        // Create and configure standard analytics components
        Composite dataCollector = CompositeFactory.createComposite("data-collector", CompositeType.OBSERVER, environment);
        Composite analyzer = CompositeFactory.createComposite("analyzer", CompositeType.TRANSFORMER, environment);
        Composite reporter = CompositeFactory.createComposite("reporter", CompositeType.OBSERVER, environment);
    ...
    public static Machine createMonitoring(String name, String description, Environment environment) {
        ...
        Machine machine = Machine.create(machineId, MachineType.MONITORING, name, description, "1.0.0", environment);
        MACHINE_REGISTRY.put(machineId, machine);
        
        // Create and configure standard monitoring components
        Composite observer = CompositeFactory.createComposite("system-observer", CompositeType.OBSERVER, environment);
        Composite validator = CompositeFactory.createComposite("validator", CompositeType.VALIDATOR, environment);
        Composite alerter = CompositeFactory.createComposite("alerter", CompositeType.OBSERVER, environment);
    ...
    public static Machine createWorkflow(String name, String description, Environment environment) {
        ...
        Machine machine = Machine.create(machineId, MachineType.WORKFLOW, name, description, "1.0.0", environment);
        MACHINE_REGISTRY.put(machineId, machine);
        
        // Create and configure standard workflow components
        Composite trigger = CompositeFactory.createComposite("trigger", CompositeType.OBSERVER, environment);
        Composite orchestrator = CompositeFactory.createComposite("orchestrator", CompositeType.TRANSFORMER, environment);
        Composite executor = CompositeFactory.createComposite("executor", CompositeType.TRANSFORMER, environment);
        Composite reporter = CompositeFactory.createComposite("reporter", CompositeType.OBSERVER, environment);
        ...
        // Connect composites
        machine.connect("trigger", "orchestrator")
     ...
     */
    public static Machine createMachine(MachineType type, String name, String description, Environment environment) {
    ...
    public static boolean removeMachine(String machineId) {
        ...
        if (machine != null) {
            try {
                ...
                LOGGER.warn("Failed to cleanly stop machine {} during removal: {}", machineId, e.getMessage());
                return true;
            }
        }
        return false;
