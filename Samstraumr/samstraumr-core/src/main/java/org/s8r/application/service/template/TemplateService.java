...
public class TemplateService {
  ...
  public boolean setDefaultEngine(String engineName) {
    ...
    if (!result.isSuccessful()) {
      logger.warn("Failed to set default template engine to {}: {}", 
          engineName, result.getMessage());
      return false;
    }
  ...
  public boolean registerTemplate(String templateKey, String templateContent) {
    ...
    if (!result.isSuccessful()) {
      logger.warn("Failed to register template {}: {}", 
          templateKey, result.getMessage());
      return false;
    }
  ...
  public boolean registerTemplate(String templateKey, String templateContent, String engineName) {
    ...
    if (!result.isSuccessful()) {
      logger.warn("Failed to register template {} with engine {}: {}", 
          templateKey, engineName, result.getMessage());
      return false;
    }
  ...
  public boolean registerTemplateFromFile(String templateKey, Path templatePath) {
    ...
    if (!result.isSuccessful()) {
      logger.warn("Failed to register template {} from file {}: {}", 
          templateKey, templatePath, result.getMessage());
      return false;
    }
   ...
   * @return True if the operation was successful, false otherwise
   */
  public boolean registerTemplateFromFile(String templateKey, Path templatePath, String engineName) {
    logger.debug("Registering template from file with engine: {}, {}, {}", 
        templateKey, templatePath, engineName);
    TemplateResult result = templatePort.registerTemplateFromFile(templateKey, templatePath, engineName);
    
    if (!result.isSuccessful()) {
      logger.warn("Failed to register template {} from file {} with engine {}: {}", 
          templateKey, templatePath, engineName, result.getMessage());
      return false;
    }
    
    logger.debug("Template registered successfully from file with engine: {}, {}, {}", 
        templateKey, templatePath, engineName);
    return true;
  }
   ...
   * @return A map of successfully registered template keys to their template info, empty if operation failed
   */
  public Map<String, TemplateInfo> registerTemplatesFromDirectory(Path directory, String filePattern) {
    ...
    if (!result.isSuccessful()) {
      logger.warn("Failed to register templates from directory {}: {}", 
          directory, result.getMessage());
      return Collections.emptyMap();
    }
    
    if (result.getFailedCount() > 0) {
      logger.warn("Some templates failed to register: {} succeeded, {} failed", 
    ...
    logger.debug("Successfully registered {} templates from directory {}", 
        result.getSuccessfulCount(), directory);
  ...
  public Map<String, TemplateInfo> registerTemplatesFromDirectory(
    ...
    logger.debug("Registering templates from directory with engine: {}, pattern: {}, engine: {}", 
        directory, filePattern, engineName);
    TemplateRegistrationResult result = 
        templatePort.registerTemplatesFromDirectory(directory, filePattern, engineName);
    
    if (!result.isSuccessful()) {
      logger.warn("Failed to register templates from directory {} with engine {}: {}", 
          directory, engineName, result.getMessage());
      return Collections.emptyMap();
    }
    ...
    logger.debug("Successfully registered {} templates from directory {} with engine {}", 
        result.getSuccessfulCount(), directory, engineName);
    return result.getTemplates();
  }
   ...
   * @return An Optional containing the rendered content if successful, empty otherwise
   */
  public Optional<String> renderTemplate(String templateKey, Map<String, Object> dataModel, String engineName) {
    ...
    if (!result.isSuccessful()) {
      logger.warn("Failed to render template {} with engine {}: {}", 
          templateKey, engineName, result.getMessage());
      return Optional.empty();
    }
   ...
   * @return An Optional containing the rendered content if successful, empty otherwise
   */
  public Optional<String> renderString(String templateContent, Map<String, Object> dataModel, String engineName) {
  ...
  public boolean generatePdf(String html, Path outputPath) {
    ...
    logger.debug("PDF generated successfully to {}, size: {} bytes", 
        outputPath, result.getDocumentSize().orElse(0L));
    return true;
  }
  ...
  public boolean generatePdf(String html, Path outputPath, PdfOptions options) {
    ...
    if (!result.isSuccessful()) {
      logger.warn("Failed to generate PDF with options to {}: {}", 
          outputPath, result.getMessage());
      return false;
    }
    
    logger.debug("PDF generated successfully with options to {}, size: {} bytes", 
        outputPath, result.getDocumentSize().orElse(0L));
    return true;
  }
  ...
  public boolean generatePdf(String html, OutputStream outputStream) {
    ...
    logger.debug("PDF generated successfully to output stream, size: {} bytes", 
        result.getDocumentSize().orElse(0L));
    return true;
  }

  /**
   * Generates a PDF document from a rendered HTML template with options and writes it to an output stream.
   *
   * @param html The HTML content
   * @param outputStream The output stream to write the PDF to
   * @param options The PDF generation options
  ...
  public boolean generatePdf(String html, OutputStream outputStream, PdfOptions options) {
    ...
    if (!result.isSuccessful()) {
      logger.warn("Failed to generate PDF with options to output stream: {}", 
          result.getMessage());
      return false;
    }
    
    logger.debug("PDF generated successfully with options to output stream, size: {} bytes", 
        result.getDocumentSize().orElse(0L));
    return true;
  }
  ...
  public boolean generateDocument(
    ...
    logger.debug("Generating {} document from template {} to: {}", 
        outputFormat, templateKey, outputPath);
    DocumentResult result = 
        templatePort.generateDocument(templateKey, dataModel, outputFormat, outputPath);
    
    if (!result.isSuccessful()) {
      logger.warn("Failed to generate {} document from template {} to {}: {}", 
          outputFormat, templateKey, outputPath, result.getMessage());
      return false;
    }
    
    logger.debug("{} document generated successfully from template {} to {}, size: {} bytes", 
        outputFormat, templateKey, outputPath, result.getDocumentSize().orElse(0L));
    return true;
  }
  ...
  public boolean generateDocument(
    ...
    logger.debug("Generating {} document with options from template {} to: {}", 
        outputFormat, templateKey, outputPath);
    DocumentResult result = 
        templatePort.generateDocument(templateKey, dataModel, outputFormat, outputPath, options);
    
    if (!result.isSuccessful()) {
      logger.warn("Failed to generate {} document with options from template {} to {}: {}", 
          outputFormat, templateKey, outputPath, result.getMessage());
      return false;
    }
    
    logger.debug("{} document generated successfully with options from template {} to {}, size: {} bytes", 
        outputFormat, templateKey, outputPath, result.getDocumentSize().orElse(0L));
    return true;
  }
  ...
  public boolean generateDocument(
    ...
    logger.debug("Generating {} document from template {} to output stream", 
        outputFormat, templateKey);
    DocumentResult result = 
        templatePort.generateDocument(templateKey, dataModel, outputFormat, outputStream);
    
    if (!result.isSuccessful()) {
      logger.warn("Failed to generate {} document from template {} to output stream: {}", 
          outputFormat, templateKey, result.getMessage());
      return false;
    }
    
    logger.debug("{} document generated successfully from template {} to output stream, size: {} bytes", 
        outputFormat, templateKey, result.getDocumentSize().orElse(0L));
    return true;
  }

  /**
   * Generates a document from a template and data model with options and writes it to an output stream.
   *
   * @param templateKey The template key/name
   * @param dataModel The data model
   * @param outputFormat The output format
  ...
  public boolean generateDocument(
      ...
      DocumentOptions options) {
    logger.debug("Generating {} document with options from template {} to output stream", 
        outputFormat, templateKey);
    DocumentResult result = 
        templatePort.generateDocument(templateKey, dataModel, outputFormat, outputStream, options);
    
    if (!result.isSuccessful()) {
      logger.warn("Failed to generate {} document with options from template {} to output stream: {}", 
          outputFormat, templateKey, result.getMessage());
      return false;
    }
    
    logger.debug("{} document generated successfully with options from template {} to output stream, size: {} bytes", 
        outputFormat, templateKey, result.getDocumentSize().orElse(0L));
    return true;
  }
   ...
   * @return True if the operation was successful, false otherwise
   */
  public boolean renderAndSaveToFile(String templateKey, Map<String, Object> dataModel, Path outputPath) {
    ...
    try {
      ...
      logger.error("Failed to save rendered content to file {}: {}", 
   ...
   * @param templateKey The template key/name
   * @param dataModel The data model
   * @return A CompletableFuture that will contain the rendered content if successful,
   *     or will complete exceptionally if unsuccessful
   */
  public CompletableFuture<String> renderTemplateAsync(String templateKey, Map<String, Object> dataModel) {
   ...
   * @param outputFormat The output format
   * @param outputPath The path where to save the document
   * @return A CompletableFuture that will complete successfully if the document was generated,
   *     or will complete exceptionally if unsuccessful
   */
