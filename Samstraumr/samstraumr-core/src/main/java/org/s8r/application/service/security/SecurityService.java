...
public class SecurityService {
  ...
  public Optional<SecurityPort.SecurityToken> authenticateWithFactors(
    ...
    if (!result.isSuccessful()) {
      logger.warn("Authentication with factors failed for user {}: {}", 
  ...
  public boolean hasPermission(String userId, String permission) {
    ...
    if (!result.isSuccessful()) {
      logger.warn("Permission check failed for user {}: {}", 
  ...
  public Optional<SecurityPort.User> createUser(
    ...
    SecurityPort.UserOperationResult result = 
        securityPort.createUser(username, password, userData);
    
    if (!result.isSuccessful()) {
      logger.warn("User creation failed for {}: {}", 
          username, result.getMessage());
      return Optional.empty();
    }
  ...
  public boolean updatePassword(String userId, String currentPassword, String newPassword) {
    ...
    if (!result.isSuccessful()) {
      logger.warn("Password update failed for user {}: {}", 
          userId, result.getMessage());
      return false;
    }
  ...
  public boolean resetPassword(String resetToken, String newPassword) {
    ...
    SecurityPort.PasswordUpdateResult result = 
        securityPort.resetPassword(resetToken, newPassword);
  ...
  public Optional<String> generatePasswordResetToken(String username) {
    ...
    SecurityPort.TokenGenerationResult result = 
        securityPort.generatePasswordResetToken(username);
    
    if (!result.isSuccessful()) {
      logger.warn("Password reset token generation failed for user {}: {}", 
          username, result.getMessage());
      return Optional.empty();
    }
  ...
  public Collection<SecurityPort.AuditLogEntry> getAuditLogs(
    ...
    logger.debug("Getting audit logs with filters: userId={}, eventTypes={}, startTime={}, endTime={}, limit={}",
        userId.orElse("any"),
    ...
    try {
      Collection<SecurityPort.AuditLogEntry> logs = securityPort.getAuditLogs(
          userId, eventTypes, startTime, endTime, limit);
  ...
  public boolean logSecurityEvent(String eventType, String userId, String action, String result) {
    logger.debug("Logging security event: type={}, userId={}, action={}, result={}",
        eventType, userId, action, result);
        
    SecurityPort.AuditLogEntry.Builder builder = createAuditLogEntryBuilder()
        .eventType(eventType)
        .userId(userId)
        .action(action)
        .result(result)
  ...
  public boolean logSecurityEvent(
    ...
    logger.debug("Logging security event with details: type={}, userId={}, action={}, result={}",
        eventType, userId, action, result);
  ...
  public CompletableFuture<SecurityPort.User> validateTokenAsync(String token) {
    return CompletableFuture.supplyAsync(() -> {
      ...
      return user.orElseThrow(() -> 
          new SecurityException("Token validation failed"));
    });
  }
}