...
@StreamTest
@Testcontainers
public class TubeStreamTest {

  private static final Logger LOGGER = LoggerFactory.getLogger(TubeStreamTest.class);
  ...
  void setUp() throws SQLException {
    ...
    composite = new Composite("stream-test-composite", environment);
    ...
    composite.connect("source", "processor").connect("processor", "sink");
  ...
  void shouldWriteProcessedDataToDatabase() throws SQLException {
    ...
    // When
    Optional<String> result = composite.process("source", inputData);
  ...
  void shouldHandleDatabaseConnectionFailure() throws SQLException {
    ...
    composite.enableCircuitBreaker("sink", 2, 1000);
    composite.addTransformer(
        "sink",
        (Function<String, String>)
            data -> {
    ...
    Map<String, Composite.CircuitBreaker> breakers = composite.getCircuitBreakers();
