/*
Filename: MachineDataFlowSteps.java
Purpose: Step definitions for machine data flow Cucumber tests.
Goals:
  - Implement Cucumber steps that verify end-to-end data flow through machines
  - Test resilience, performance, and state tracking of machines
  - Demonstrate machine-level testing approach with Cucumber
Dependencies:
  - Cucumber for BDD testing
  - Bundle and Machine classes for end-to-end testing
Assumptions:
  - Each scenario starts with a clean environment
  - Test environment has sufficient resources for performance tests
*/
...
/**
 * Step definitions for machine data flow tests.
 * These steps implement the Cucumber scenarios for testing
 * end-to-end data flow through machines composed of connected bundles.
 */
@MachineTest
public class MachineDataFlowSteps {
    ...
    public void all_machine_components_should_have_valid_identifiers() {
        ...
        LOGGER.info("Verified valid identifiers for {} bundles and {} tubes", 
                    bundleIds.size(), tubeIds.size());
    }
    
    @Then("the machine should contain exactly {int} bundles")
    public void the_machine_should_contain_exactly_bundles(Integer count) {
        assertEquals(count.intValue(), bundles.size(), 
                    "Machine should contain exactly " + count + " bundles");
    }
    ...
    public void the_bundles_should_be_properly_connected_in_sequence() {
        ...
        try {
            ...
            machineState.put("connection_map", Map.of(
    ...
    public void the_source_bundle_generates_data_elements(Integer count) {
        // Generate test data
        sourceData = IntStream.range(0, count)
            .mapToObj(i -> "data-element-" + i)
    ...
    public void the_transformation_bundle_converts_all_text_to_uppercase() {
        ...
        processorBundle.addTransformer("transformer", 
    ...
    public void the_transformation_bundle_is_configured_to_fail_on_the_element(Integer elementIndex) {
        ...
        processorBundle.addTransformer("transformer", (Function<String, String>) data -> {
    ...
    public void circuit_breakers_are_enabled_for_all_bundles() {
        for (Map.Entry<String, Bundle> entry : bundles.entrySet()) {
            ...
            for (String tubeName : bundle.getTubes().keySet()) {
                ...
                LOGGER.info("Enabled circuit breaker for tube {} in bundle {}", 
                           tubeName, bundleName);
            }
        }
    }
        ...
        try {
            ...
            for (String data : sourceData) {
                try {
                    ...
                    lastException = e;
                    LOGGER.error("Error processing data element {}: {}", 
                               data, e.getMessage());
                }
            }
            
            LOGGER.info("Executed machine data flow, processed {} out of {} elements", 
                      processedData.size(), sourceData.size());
    ...
    public void the_sink_bundle_should_receive_exactly_data_elements(Integer count) {
        assertEquals(count.intValue(), processedData.size(), 
    ...
    public void all_output_data_should_be_uppercase() {
        for (String data : processedData) {
            assertEquals(data.toUpperCase(), data, 
                       "All output data should be uppercase");
        }
    }
    ...
    public void the_machine_execution_report_should_indicate_a_partial_success() {
        ...
        assertTrue(processedData.size() < sourceData.size(), 
                 "Not all data should be processed");
        
        LOGGER.info("Verified partial success: {} of {} elements processed",
                  processedData.size(), sourceData.size());
    }
    ...
    public void the_failure_should_be_properly_isolated_to_the_transformation_bundle() {
        ...
        assertTrue(lastException.getMessage().contains("Simulated failure"), 
    ...
    public void all_data_should_be_processed_successfully() {
        assertEquals(sourceData.size(), processedData.size(), 
    ...
    public void the_sink_bundle_should_receive_all_data_elements_with_metadata(Integer count) {
        ...
        assertEquals(count.intValue(), dataWithMetadata.size(), 
