...
Purpose: Integration tests for data flowing through individual tubes.
Goals:
  - Verify that data processing within tubes behaves as expected
  - Test data transformation, validation, and state tracking
  - Ensure tube logging properly records data flow events
Dependencies:
  - JUnit 5 for test framework
  - Mockito for mocking environment and external dependencies
  - Tube.java for the component under test
Assumptions:
  - Tests are isolated from other system components
  - Mock data provides realistic representations of actual data flows
*/
...
import org.samstraumr.tube.annotations.FlowTest;

/**
 * Flow Tests focus on data flowing through a single tube.
 * These tests follow the integration testing approach, verifying
 * data transformation, validation, and state tracking.
 */
@FlowTest
...
public class FlowTest {
    ...
    void shouldHandleProcessingError() {
        ...
        Function<String, String> errorProcessor = data -> {
    ...
    private <T, R> R processDataWithLineage(Tube tube, T input, Function<T, R> processor, String reason) {
        // Add to tube lineage
