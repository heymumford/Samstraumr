/*
Filename: ImproveBundleTest.java
Purpose: Component tests for connected tubes forming bundles.
Goals:
  - Verify that tubes can be connected correctly and data flows through them
  - Test bundle-level operations like activation/deactivation and circuit breaking
  - Ensure transformers and validators work correctly within bundles
Dependencies:
  - JUnit 5 for test framework
  - Bundle.java and Tube.java for the components under test
Assumptions:
  - Bundle operations are tested in isolation from external systems
  - Tests verify component behaviors and interactions
*/
...
/**
 * Bundle Tests focus on connected tubes forming bundles.
 * These tests verify component interactions and data flow through
 * connected tubes in a bundle.
 */
@BundleTest
...
public class ImproveBundleTest {
    ...
    void shouldCorrectlyConnectTubes() {
        ...
        bundle.connect("source", "processor1")
    ...
    void shouldHandleValidationFailures() {
        ...
        bundle.connect("source", "validator")
              .connect("validator", "sink");
    ...
    void circuitBreakerShouldOpenAfterFailures() {
        ...
        bundle.addTransformer("processor", (Function<String, String>) input -> {
    ...
    void shouldHandleParallelProcessing() throws InterruptedException {
        ...
        bundle.addTransformer("sink", (Function<String, String>) input -> {
            try {
        ...
        List<Bundle.BundleEvent> events = bundle.getEventLog();
        assertTrue(events.size() >= THREAD_COUNT * 2); // At least start and complete events for each thread
    }
}