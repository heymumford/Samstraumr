...
/**
 * Step definitions for testing the Tube Lifecycle model following a biological development approach.
 *
...
public class TubeLifecycleSteps extends BaseLifecycleSteps {
    ...
    public void the_identifier_should_include_a_creation_timestamp() {
        ...
        boolean hasTimestamp = logs.stream()
            .anyMatch(entry -> entry.matches("^\\d{4}-\\d{2}-\\d{2}T.*"));
    ...
    public void the_identifier_should_not_be_modifiable() {
        ...
        assertEquals(originalId, testTube.getUniqueId(), 
            "Tube ID should be immutable");
    }
