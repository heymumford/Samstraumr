...
public class InMemoryCacheAdapterTest {
    ...
    void initialize_shouldReturnSuccessResult() {
        ...
        assertEquals("Cache already initialized", result.getMessage());
    ...
    void put_shouldStoreValueSuccessfully() {
        ...
        // Act
        CachePort.CacheResult result = adapter.put(TEST_REGION, key, value);
    ...
    void put_withNullValueShouldRemoveKey() {
        ...
        String key = "testKey";
        String value = "Test Value";
        adapter.put(TEST_REGION, key, value);
        
        // Act - Put null value
        CachePort.CacheResult result = adapter.put(TEST_REGION, key, null);
    ...
    void getOrCompute_shouldReturnCachedValue() {
        ...
        // Act
        CachePort.CacheResult result = adapter.getOrCompute(TEST_REGION, key, String.class, provider);
    ...
    void getOrCompute_shouldReturnFailureWhenProviderReturnsNull() {
        ...
        assertFalse(result.isSuccessful());
        assertTrue(result.getReason().isPresent());
        assertTrue(result.getReason().get().contains("Computation returned null"));
    }
    ...
    void getKeys_shouldReturnAllValidKeys() {
        // Arrange
        adapter.put(TEST_REGION, "key1", "value1");
        adapter.put(TEST_REGION, "key2", "value2");
        adapter.put(TEST_REGION, "key3", "value3");
    ...
    void clearRegion_shouldRemoveAllEntriesInRegion() {
        ...
        adapter.put(TEST_REGION, "key1", "value1");
        adapter.put(TEST_REGION, "key2", "value2");
        adapter.put(CachePort.CacheRegion.COMPONENT, "key3", "value3");
        ...
        assertTrue(adapter.containsKey(CachePort.CacheRegion.COMPONENT, "key3")); // Should not be affected
    }
    ...
    void clearAll_shouldRemoveAllEntries() {
        // Arrange
        adapter.put(TEST_REGION, "key1", "value1");
        adapter.put(CachePort.CacheRegion.COMPONENT, "key2", "value2");
        ...
        assertTrue(result.isSuccessful());
        assertFalse(adapter.containsKey(TEST_REGION, "key1"));
        assertFalse(adapter.containsKey(CachePort.CacheRegion.COMPONENT, "key2"));
    }
    ...
    void invalidParameters_shouldReturnFailureResults() {
        // Act & Assert - For put
        CachePort.CacheResult putResult = adapter.put(null, "key", "value");
