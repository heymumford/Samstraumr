...
public class NotificationServiceTest {
    ...
    void setUp() {
        ...
        when(mockComponent.getId()).thenReturn(new ComponentId("test-component-123"));
        when(mockComponent.getName()).thenReturn("Test Component");
        when(mockComponent.getType()).thenReturn("TestComponent");
        when(mockComponent.getState()).thenReturn(Component.State.ACTIVE);
    }
    
    @Test
    void testSendComponentStatusNotification() {
        // Setup mock notification result
        NotificationResult expectedResult = new NotificationResult("test-notif-123", 
                DeliveryStatus.SENT, "Notification sent successfully");
        when(mockNotificationPort.sendSystemNotification(anyString(), anyString(), 
                eq(NotificationSeverity.INFO))).thenReturn(expectedResult);
        
        // Call the service method
        NotificationResult result = service.sendComponentStatusNotification(mockComponent, "Status update");
        ...
        verify(mockNotificationPort).sendSystemNotification(
    ...
    void testSendComponentErrorNotification() {
        // Setup mock notification result
        NotificationResult expectedResult = new NotificationResult("test-notif-456", 
                DeliveryStatus.SENT, "Notification sent successfully");
        when(mockNotificationPort.sendSystemNotification(anyString(), anyString(), 
                eq(NotificationSeverity.ERROR))).thenReturn(expectedResult);
        
        // Call the service method
        NotificationResult result = service.sendComponentErrorNotification(mockComponent, "Test error message");
    ...
    void testSendMachineCreationNotification() {
        // Setup mock notification result
        NotificationResult expectedResult = new NotificationResult("test-notif-789", 
                DeliveryStatus.SENT, "Notification sent successfully");
        when(mockNotificationPort.sendSystemNotification(anyString(), anyString(), 
                eq(NotificationSeverity.INFO))).thenReturn(expectedResult);
        
        // Call the service method
        NotificationResult result = service.sendMachineCreationNotification(
                "machine-123", MachineType.PROCESSOR, "Test Machine");
    ...
    void testSendSystemWarning() {
        // Setup mock notification result
        NotificationResult expectedResult = new NotificationResult("test-notif-abc", 
                DeliveryStatus.SENT, "Notification sent successfully");
        when(mockNotificationPort.sendSystemNotification(anyString(), anyString(), 
                eq(NotificationSeverity.WARNING))).thenReturn(expectedResult);
    ...
    void testSendSystemError() {
        // Setup mock notification result
        NotificationResult expectedResult = new NotificationResult("test-notif-def", 
                DeliveryStatus.SENT, "Notification sent successfully");
        when(mockNotificationPort.sendSystemNotification(anyString(), anyString(), 
                eq(NotificationSeverity.ERROR))).thenReturn(expectedResult);
    ...
    void testSendUserNotification_RegisteredUser() {
        // Setup mock notification result
        NotificationResult expectedResult = new NotificationResult("test-notif-ghi", 
                DeliveryStatus.SENT, "Notification sent successfully");
        ...
        when(mockNotificationPort.sendNotificationToRecipient(
                ...
                any(NotificationSeverity.class), anyMap())).thenReturn(expectedResult);
        
        // Call the service method
        NotificationResult result = service.sendUserNotification(
        ...
        verify(mockNotificationPort).sendNotificationToRecipient(
                eq("user123"), 
                eq("User Subject"),
                eq("User message"),
                eq(NotificationSeverity.INFO),
    ...
    void testSendUserNotification_UnregisteredUser() {
        // Setup mock notification result
        NotificationResult expectedResult = new NotificationResult("test-notif-jkl", 
                DeliveryStatus.SENT, "Notification sent successfully");
        ...
        when(mockNotificationPort.sendNotificationToRecipient(
                eq("user456"), anyString(), anyString(), 
                any(NotificationSeverity.class), anyMap())).thenReturn(expectedResult);
        ...
        verify(mockNotificationPort).registerRecipient(
                eq("user456"), 
    ...
    void testRegisterUserRecipient() {
        ...
        when(mockNotificationPort.registerRecipient(
                eq("user789"), argThat(map -> map.containsKey("type") && map.get("type").equals("email"))))
                .thenReturn(true);
        ...
        verify(mockNotificationPort).registerRecipient(
                eq("user789"), 
                argThat(map -> 
                    map.containsKey("type") && map.get("type").equals("email") && 
                    map.containsKey("address") && map.get("address").equals("user789@example.com")));
        verify(mockLogger).debug(anyString(), anyString());
    }
    ...
    void testSendPushNotification_RegisteredUser() {
        // Setup mock notification result
        NotificationResult expectedResult = new NotificationResult("test-notif-push", 
                DeliveryStatus.SENT, "Notification sent successfully");
        when(mockNotificationPort.isRecipientRegistered("push-user")).thenReturn(true);
        when(mockNotificationPort.sendNotificationToRecipient(
                eq("push-user"), anyString(), anyString(), 
                any(NotificationSeverity.class), argThat(map -> map.containsKey("channel") && map.get("channel").equals("push"))))
        ...
        // Call the service method
        NotificationResult result = service.sendPushNotification(
        ...
        verify(mockNotificationPort).sendNotificationToRecipient(
                ...
                eq(NotificationSeverity.INFO),
                argThat(map -> 
                    map.containsKey("channel") && map.get("channel").equals("push") &&
                    map.containsKey("action") && map.get("action").equals("open_profile") &&
                    map.containsKey("profileId") && map.get("profileId").equals("12345")));
        verify(mockLogger).debug(anyString(), anyString());
    }
    ...
    void testSendPushNotification_UnregisteredUser() {
        ...
        verify(mockNotificationPort, never()).sendNotificationToRecipient(
    ...
    void testRegisterPushDevice() {
        // Setup mock
        when(mockNotificationPort.registerRecipient(
                eq("device-user"), argThat(map -> 
                    map.containsKey("type") && map.get("type").equals("push") &&
                    map.containsKey("device") && map.get("device").equals("device-token-123"))))
                .thenReturn(true);
        
        // Call the service method
        boolean result = service.registerPushDevice(
                "device-user", "device-token-123", "android", "1.0.0");
        ...
        verify(mockNotificationPort).registerRecipient(
                ...
                argThat(map -> 
                    map.containsKey("type") && map.get("type").equals("push") &&
                    map.containsKey("device") && map.get("device").equals("device-token-123") &&
                    map.containsKey("platform") && map.get("platform").equals("android") &&
                    map.containsKey("appVersion") && map.get("appVersion").equals("1.0.0") &&
    ...
    void testSendNotificationAsync() throws Exception {
        // Setup mock notification result
        NotificationResult expectedResult = new NotificationResult("test-notif-async", 
                DeliveryStatus.SENT, "Notification sent successfully");
        ...
        // Call the service method
        CompletableFuture<NotificationResult> future = service.sendNotificationAsync(
        ...
        // Verify the mock interactions
        verify(mockNotificationPort).sendNotification(
                eq("Async Subject"), 
                eq("Async content"),
                eq(NotificationSeverity.INFO),
                anyMap());
    ...
    void testSendBatchNotification() {
        ...
        NotificationResult result1 = new NotificationResult("notif-1", 
                DeliveryStatus.SENT, "Notification sent successfully");
        NotificationResult result2 = new NotificationResult("notif-2", 
                DeliveryStatus.SENT, "Notification sent successfully");
        NotificationResult result3 = new NotificationResult("notif-3", 
                DeliveryStatus.FAILED, "Recipient not found");
        ...
        // Call the service method
        Map<String, NotificationResult> results = service.sendBatchNotification(
        ...
        // Verify the mock interactions
        verify(mockNotificationPort).sendNotificationToRecipient(
                eq("user1"), eq("Batch Subject"), eq("Batch content"), 
                eq(NotificationSeverity.INFO), eq(metadata));
        verify(mockNotificationPort).sendNotificationToRecipient(
                eq("user2"), eq("Batch Subject"), eq("Batch content"), 
                eq(NotificationSeverity.INFO), eq(metadata));
        verify(mockNotificationPort).sendNotificationToRecipient(
                eq("user3"), eq("Batch Subject"), eq("Batch content"), 
                eq(NotificationSeverity.INFO), eq(metadata));
        verify(mockLogger).debug(anyString());
    }
}