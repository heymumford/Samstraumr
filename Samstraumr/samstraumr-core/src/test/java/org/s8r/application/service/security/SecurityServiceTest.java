...
class SecurityServiceTest {
    ...
    void getRoles_shouldReturnEmptySet_whenExceptionOccurs() {
        ...
        verify(logger).debug(contains("Getting roles"), eq(userId));
        verify(logger).error(contains("Failed to get roles"), eq(userId), anyString(), any(RuntimeException.class));
    }
    ...
    void getPermissions_shouldReturnEmptySet_whenExceptionOccurs() {
        ...
        assertTrue(result.isEmpty());
        verify(logger).debug(contains("Getting permissions"), eq(userId));
        verify(logger).error(contains("Failed to get permissions"), eq(userId), anyString(), any(RuntimeException.class));
    }
    ...
    void createUser_withUserData_shouldReturnUser_whenCreationSucceeds() {
        ...
        String username = "newUser";
        String password = "password123";
        Map<String, String> userData = Map.of("email", "user@example.com", "firstName", "Test", "lastName", "User");
        User mockUser = mock(User.class);
    ...
    void updatePassword_shouldReturnTrue_whenUpdateSucceeds() {
        ...
        when(securityPort.updatePassword(userId, currentPassword, newPassword))
                .thenReturn(PasswordUpdateResult.success("Password updated successfully", false, Optional.empty()));
    ...
    void updatePassword_shouldReturnFalse_whenUpdateFails() {
        ...
        when(securityPort.updatePassword(userId, currentPassword, newPassword))
                .thenReturn(PasswordUpdateResult.failure("Current password is incorrect", "INVALID_CREDENTIALS"));
    ...
    void resetPassword_shouldReturnTrue_whenResetSucceeds() {
        ...
        when(securityPort.resetPassword(resetToken, newPassword))
                .thenReturn(PasswordUpdateResult.success("Password reset successfully", false, Optional.empty()));
    ...
    void generatePasswordResetToken_shouldReturnEmpty_whenGenerationFails() {
        ...
        assertFalse(result.isPresent());
        verify(logger).debug(contains("Generating password reset token"), eq(username));
        verify(logger).warn(contains("Password reset token generation failed"), eq(username), anyString());
    }
    ...
    void encrypt_shouldReturnEncryptedData_whenEncryptionSucceeds() {
        ...
        when(securityPort.encrypt(eq(data), any()))
                .thenReturn(EncryptionResult.success("Data encrypted successfully", encryptedData, "defaultKey"));
    ...
    void decrypt_shouldReturnDecryptedData_whenDecryptionSucceeds() {
        ...
        when(securityPort.decrypt(eq(encryptedData), any()))
                .thenReturn(DecryptionResult.success("Data decrypted successfully", decryptedData, "defaultKey"));
    ...
    void getAuditLogs_shouldReturnLogs() {
        ...
        when(securityPort.getAuditLogs(userId, eventTypes, startTime, endTime, limit)).thenReturn(mockLogs);

        // Act
        Collection<AuditLogEntry> result = service.getAuditLogs(userId, eventTypes, startTime, endTime, limit);

        // Assert
        assertEquals(mockLogs, result);
        verify(logger).debug(contains("Getting audit logs"), anyString(), anyString(), anyString(), anyString(), anyString());
    ...
    void getAuditLogs_shouldReturnEmptyList_whenExceptionOccurs() {
        ...
        assertTrue(result.isEmpty());
        verify(logger).debug(contains("Getting audit logs"), anyString(), anyString(), anyString(), anyString(), anyString());
        verify(logger).error(contains("Failed to get audit logs"), anyString(), any(RuntimeException.class));
    }
    ...
    void logSecurityEvent_shouldReturnTrue_whenLoggingSucceeds() {
        ...
        verify(logger).debug(contains("Logging security event"), eq(eventType), eq(userId), eq(action), eq(result));
    ...
    void logSecurityEvent_withDetails_shouldReturnTrue_whenLoggingSucceeds() {
        ...
        // Assert
        assertTrue(logResult);
        verify(logger).debug(contains("Logging security event with details"), eq(eventType), eq(userId), eq(action), eq(result));
        verify(securityPort).logSecurityEvent(any(AuditLogEntry.class));
    }

    @Test
    void authenticateAsync_shouldCompleteSuccessfully_whenCredentialsAreValid() throws ExecutionException, InterruptedException {
        // Arrange
        String username = "testUser";
        String password = "password123";
        SecurityToken mockToken = mock(SecurityToken.class);
        ...
        SecurityToken result = future.get(); // This will throw an exception if the future completes exceptionally
    ...
    @Test
    void validateTokenAsync_shouldCompleteSuccessfully_whenTokenIsValid() throws ExecutionException, InterruptedException {
        // Arrange
        String token = "validToken";
        SecurityToken mockToken = mock(SecurityToken.class);
        User mockUser = mock(User.class);
        ...
        User result = future.get(); // This will throw an exception if the future completes exceptionally
