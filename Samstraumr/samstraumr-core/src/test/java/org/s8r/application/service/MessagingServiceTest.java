...
public class MessagingServiceTest {
    ...
    void initialize_shouldCallPortInitialize() {
        // Arrange
        when(mockMessagingPort.initialize()).thenReturn(MessagingPort.MessageResult.success(null, "Success"));
    ...
    void initialize_shouldLogErrorOnFailure() {
        // Arrange
        when(mockMessagingPort.initialize()).thenReturn(
    ...
    void createChannel_shouldReturnChannelOnSuccess() {
        ...
        when(mockMessagingPort.createChannel(name, type)).thenReturn(
    ...
    void createChannel_shouldReturnEmptyAndLogOnFailure() {
        ...
        MessagingPort.ChannelType type = MessagingPort.ChannelType.TOPIC;
        when(mockMessagingPort.createChannel(name, type)).thenReturn(
                MessagingPort.MessageResult.failure(null, "Failed to create channel", "Channel already exists")
        );
    ...
    void createChannel_withPropertiesShouldReturnChannelOnSuccess() {
        ...
        when(mockMessagingPort.createChannel(name, type, properties)).thenReturn(
                MessagingPort.MessageResult.success(null, "Channel created successfully", attributes)
        );
    ...
    void getChannel_shouldReturnChannelOnSuccess() {
        ...
        when(mockMessagingPort.getChannel(name)).thenReturn(
                MessagingPort.MessageResult.success(null, "Channel found", attributes)
        );
    ...
    void getChannel_shouldReturnEmptyOnNotFound() {
        ...
        when(mockMessagingPort.getChannel(name)).thenReturn(
                MessagingPort.MessageResult.failure(null, "Channel not found", 
                        "No channel exists with name 'nonExistentChannel'")
        );
    ...
    void getChannel_shouldLogWarningOnOtherErrors() {
        ...
        when(mockMessagingPort.getChannel(name)).thenReturn(
                MessagingPort.MessageResult.failure(null, "Error", "System error")
        );
    ...
    void deleteChannel_shouldReturnTrueOnSuccess() {
        // Arrange
        String name = "testChannel";
        when(mockMessagingPort.deleteChannel(name)).thenReturn(
                MessagingPort.MessageResult.success(null, "Channel deleted successfully")
        );
    ...
    void deleteChannel_shouldReturnFalseAndLogOnFailure() {
        ...
        String name = "nonExistentChannel";
        when(mockMessagingPort.deleteChannel(name)).thenReturn(
                MessagingPort.MessageResult.failure(null, "Failed to delete channel", "Channel not found")
        );
    ...
    void getChannels_shouldReturnChannelsOnSuccess() {
        ...
        when(mockMessagingPort.getChannels()).thenReturn(
                MessagingPort.MessageResult.success(null, "Channels retrieved successfully", attributes)
        );
    ...
    void send_shouldReturnTrueOnSuccess() {
        ...
        when(mockMessagingPort.send(topic, mockMessage)).thenReturn(
                MessagingPort.MessageResult.success("msg123", "Message sent successfully")
        );
    ...
    void send_shouldReturnFalseAndLogOnFailure() {
        ...
        when(mockMessagingPort.send(topic, mockMessage)).thenReturn(
                MessagingPort.MessageResult.failure("msg123", "Failed to send message", "No subscribers")
        );
    ...
    void send_withOptionsShouldDelegateToPort() {
        ...
        when(mockMessagingPort.send(topic, mockMessage, mockOptions)).thenReturn(
                MessagingPort.MessageResult.success("msg123", "Message sent successfully")
        );
    ...
    void subscribe_shouldReturnSubscriptionIdOnSuccess() {
        ...
        when(mockMessagingPort.subscribe(topic, mockHandler)).thenReturn(
    ...
    void subscribe_shouldReturnEmptyAndLogOnFailure() {
        ...
        when(mockMessagingPort.subscribe(topic, mockHandler)).thenReturn(
                MessagingPort.MessageResult.failure(null, "Failed to subscribe", "Maximum subscribers reached")
        );
    ...
    void unsubscribe_shouldReturnTrueOnSuccess() {
        ...
        when(mockMessagingPort.unsubscribe(topic, subscriptionId)).thenReturn(
                MessagingPort.MessageResult.success(null, "Unsubscribed successfully")
        );
    ...
    void unsubscribe_shouldReturnFalseAndLogOnFailure() {
        ...
        when(mockMessagingPort.unsubscribe(topic, subscriptionId)).thenReturn(
                MessagingPort.MessageResult.failure(null, "Failed to unsubscribe", "Subscription not found")
        );
    ...
    void request_shouldReturnReplyOnSuccess() {
        ...
        when(mockMessagingPort.request(eq(topic), eq(mockMessage), any(Duration.class))).thenReturn(
                MessagingPort.MessageResult.success("msg123", "Reply received", attributes)
        );
    ...
    void request_shouldReturnEmptyOnTimeout() {
        ...
        when(mockMessagingPort.request(eq(topic), eq(mockMessage), any(Duration.class))).thenReturn(
                MessagingPort.MessageResult.failure("msg123", "Request timed out", "No reply received within timeout period")
        );
    ...
    void request_shouldLogWarningOnOtherErrors() {
        ...
        when(mockMessagingPort.request(eq(topic), eq(mockMessage), any(Duration.class))).thenReturn(
                MessagingPort.MessageResult.failure("msg123", "Request failed", "No handler registered")
        );
    ...
    void requestAsync_shouldReturnCompletableFuture() throws Exception {
        ...
        // Act
        CompletableFuture<MessagingPort.Message> future = service.requestAsync(topic, mockMessage, Duration.ofSeconds(5));
    ...
    void registerReplyHandler_shouldReturnHandlerIdOnSuccess() {
        ...
        Function<MessagingPort.Message, MessagingPort.Message> handler = msg -> mock(MessagingPort.Message.class);
        String handlerId = "handler123";
        
        Map<String, Object> attributes = new HashMap<>();
        attributes.put("handlerId", handlerId);
        
        when(mockMessagingPort.registerReplyHandler(topic, handler)).thenReturn(
                MessagingPort.MessageResult.success(null, "Handler registered successfully", attributes)
        );
    ...
    void registerReplyHandler_shouldReturnEmptyAndLogOnFailure() {
        ...
        when(mockMessagingPort.registerReplyHandler(topic, handler)).thenReturn(
                MessagingPort.MessageResult.failure(null, "Failed to register handler", "System error")
        );
    ...
    void unregisterReplyHandler_shouldReturnTrueOnSuccess() {
        ...
        when(mockMessagingPort.unregisterReplyHandler(topic, handlerId)).thenReturn(
                MessagingPort.MessageResult.success(null, "Handler unregistered successfully")
        );
    ...
    void unregisterReplyHandler_shouldReturnFalseAndLogOnFailure() {
        ...
        when(mockMessagingPort.unregisterReplyHandler(topic, handlerId)).thenReturn(
                MessagingPort.MessageResult.failure(null, "Failed to unregister handler", "Handler not found")
        );
    ...
    void broadcast_shouldSendToMultipleTopics() {
        ...
        when(mockMessagingPort.send("topic1", mockMessage)).thenReturn(
                MessagingPort.MessageResult.success("msg123", "Message sent successfully")
        );
        when(mockMessagingPort.send("topic2", mockMessage)).thenReturn(
                MessagingPort.MessageResult.success("msg123", "Message sent successfully")
        );
        when(mockMessagingPort.send("topic3", mockMessage)).thenReturn(
    ...
    void shutdown_shouldCallPortShutdown() {
        // Arrange
        when(mockMessagingPort.shutdown()).thenReturn(
    ...
    void shutdown_shouldLogErrorOnFailure() {
        // Arrange
        when(mockMessagingPort.shutdown()).thenReturn(
                MessagingPort.MessageResult.failure(null, "Shutdown failed", "Test error")
        );
