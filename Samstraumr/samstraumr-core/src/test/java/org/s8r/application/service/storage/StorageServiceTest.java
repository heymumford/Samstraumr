...
class StorageServiceTest {
    ...
    void listContainers_shouldReturnContainers_whenListingSucceeds() {
        ...
        when(storagePort.listContainers(prefix, maxResults))
                .thenReturn(ContainerListResult.success("Containers listed successfully", mockContainers, false));
    ...
    void storeObject_shouldReturnEmpty_whenStorageFails() {
        ...
        assertFalse(result.isPresent());
        verify(logger).debug(contains("Storing object"), eq(containerName), eq(key), eq(data.length));
        verify(logger).warn(contains("Failed to store object"), eq(containerName), eq(key), anyString());
    }
    ...
    void storeObject_withOptions_shouldReturnObject_whenStorageSucceeds() {
        ...
        verify(logger).debug(contains("Object stored successfully with options"), eq(containerName), eq(key));
    }
    ...
    void getObject_withOutputStream_shouldReturnTrue_whenObjectExists() {
        ...
        verify(logger).debug(contains("Object retrieved successfully to stream"), eq(containerName), eq(key));
    }
    ...
    void getObject_withOutputStream_shouldReturnFalse_whenObjectDoesNotExist() {
        ...
        assertFalse(result);
        verify(logger).debug(contains("Getting object to stream"), eq(containerName), eq(key));
        verify(logger).warn(contains("Failed to get object to stream"), eq(containerName), eq(key), anyString());
    }
    ...
    void deleteObject_shouldReturnFalse_whenDeletionFails() {
        ...
        assertFalse(result);
        verify(logger).debug(contains("Deleting object"), eq(containerName), eq(key));
        verify(logger).warn(contains("Failed to delete object"), eq(containerName), eq(key), anyString());
    }
    ...
    void getObjectMetadata_shouldReturnMetadata_whenObjectExists() {
        ...
        when(storagePort.getObjectMetadata(containerName, key))
                .thenReturn(ObjectMetadataResult.success(
        ...
        verify(logger).debug(contains("Object metadata retrieved successfully"), eq(containerName), eq(key));
    }
    ...
    void getObjectMetadata_shouldReturnEmpty_whenObjectDoesNotExist() {
        ...
        assertFalse(result.isPresent());
        verify(logger).debug(contains("Getting object metadata"), eq(containerName), eq(key));
        verify(logger).warn(contains("Failed to get object metadata"), eq(containerName), eq(key), anyString());
    }
    ...
    void updateObjectMetadata_shouldReturnTrue_whenUpdateSucceeds() {
        ...
        // Assert
        assertTrue(result);
        verify(logger).debug(contains("Updating object metadata"), eq(containerName), eq(key), eq(replace));
        verify(logger).debug(contains("Object metadata updated successfully"), eq(containerName), eq(key));
    }
    ...
    void updateObjectMetadata_shouldReturnFalse_whenUpdateFails() {
        ...
        assertFalse(result);
        verify(logger).debug(contains("Updating object metadata"), eq(containerName), eq(key), eq(replace));
        verify(logger).warn(contains("Failed to update object metadata"), eq(containerName), eq(key), anyString());
    }
    ...
    void listObjects_shouldReturnObjects_whenListingSucceeds() {
        ...
        Optional<Integer> maxResults = Optional.of(10);
        List<StorageObject> mockObjects = Arrays.asList(mock(StorageObject.class), mock(StorageObject.class));

        when(storagePort.listObjects(containerName, prefix, delimiter, maxResults))
                .thenReturn(ObjectListResult.success(
    ...
    void copyObject_shouldReturnObject_whenCopySucceeds() {
        ...
        // Act
        Optional<StorageObject> result = service.copyObject(
                sourceContainer, sourceKey, destinationContainer, destinationKey);

        // Assert
        assertTrue(result.isPresent());
        assertSame(mockObject, result.get());
        verify(logger).debug(contains("Copying object"), eq(sourceContainer), eq(sourceKey), 
                eq(destinationContainer), eq(destinationKey));
        verify(logger).debug(contains("Object copied successfully"), eq(sourceContainer), eq(sourceKey), 
                eq(destinationContainer), eq(destinationKey));
    }
    ...
    void copyObject_shouldReturnEmpty_whenCopyFails() {
        ...
        verify(logger).debug(contains("Copying object"), eq(sourceContainer), eq(sourceKey), 
                eq(destinationContainer), eq(destinationKey));
        verify(logger).warn(contains("Failed to copy object"), eq(sourceContainer), eq(sourceKey), 
                eq(destinationContainer), eq(destinationKey), anyString());
    }
    ...
    void moveObject_shouldReturnObject_whenMoveSucceeds() {
        ...
        // Act
        Optional<StorageObject> result = service.moveObject(
                sourceContainer, sourceKey, destinationContainer, destinationKey);

        // Assert
        assertTrue(result.isPresent());
        assertSame(mockObject, result.get());
        verify(logger).debug(contains("Moving object"), eq(sourceContainer), eq(sourceKey), 
                eq(destinationContainer), eq(destinationKey));
        verify(logger).debug(contains("Object moved successfully"), eq(sourceContainer), eq(sourceKey), 
                eq(destinationContainer), eq(destinationKey));
    }
    ...
    void moveObject_shouldReturnEmpty_whenMoveFails() {
        ...
        verify(logger).debug(contains("Moving object"), eq(sourceContainer), eq(sourceKey), 
                eq(destinationContainer), eq(destinationKey));
        verify(logger).warn(contains("Failed to move object"), eq(sourceContainer), eq(sourceKey), 
                eq(destinationContainer), eq(destinationKey), anyString());
    }
    ...
    void generatePresignedUrl_shouldReturnUrl_whenGenerationSucceeds() {
        ...
        verify(logger).debug(contains("Generating presigned URL"), eq(containerName), eq(key), eq(operation), any());
        verify(logger).debug(contains("Presigned URL generated successfully"), eq(containerName), eq(key));
    }
    ...
    void generatePresignedUrl_shouldReturnEmpty_whenGenerationFails() {
        ...
        // Act
        Optional<String> result = service.generatePresignedUrl(containerName, key, operation, expiration);

        // Assert
        assertFalse(result.isPresent());
        verify(logger).debug(contains("Generating presigned URL"), eq(containerName), eq(key), eq(operation), eq(expiration));
        verify(logger).warn(contains("Failed to generate presigned URL"), eq(containerName), eq(key), anyString());
    }
    ...
    void setRetentionPolicy_shouldReturnTrue_whenSettingSucceeds() {
        ...
        // Assert
        assertTrue(result);
        verify(logger).debug(contains("Setting retention policy"), eq(containerName), eq(key), eq(retentionPeriod));
    ...
    void setRetentionPolicy_shouldReturnFalse_whenSettingFails() {
        ...
        assertFalse(result);
        verify(logger).debug(contains("Setting retention policy"), eq(containerName), eq(key), eq(retentionPeriod));
        verify(logger).warn(contains("Failed to set retention policy"), eq(containerName), eq(key), anyString());
    }
    ...
    void setContainerRetentionPolicy_shouldReturnTrue_whenSettingSucceeds() {
        ...
        // Assert
        assertTrue(result);
        verify(logger).debug(contains("Setting container retention policy"), eq(containerName), eq(retentionPeriod));
        verify(logger).debug(contains("Container retention policy set successfully"), eq(containerName));
    }
    ...
    void setContainerRetentionPolicy_shouldReturnFalse_whenSettingFails() {
        ...
        assertFalse(result);
        verify(logger).debug(contains("Setting container retention policy"), eq(containerName), eq(retentionPeriod));
        verify(logger).warn(contains("Failed to set container retention policy"), eq(containerName), anyString());
    }
    ...
    void storeKeyValue_withTtl_shouldReturnTrue_whenStorageSucceeds() {
        ...
        when(storagePort.storeKeyValue(key, value, ttl))
                .thenReturn(KvResult.success("Key-value pair stored successfully", value, 
                        Optional.of(Instant.now().plus(ttl))));
    ...
    void updateKeyValue_shouldReturnNewValue_whenUpdateSucceeds() {
        ...
        Function<Optional<String>, String> transformFunction = value -> 
                value.map(v -> v + "-updated").orElse("default");
    ...
    void createContainerAsync_shouldCompleteSuccessfully_whenCreationSucceeds() throws Exception {
        ...
        Container result = future.get(); // This will throw an exception if the future completes exceptionally
    ...
    void storeObjectAsync_shouldCompleteSuccessfully_whenStorageSucceeds() throws Exception {
        ...
        StorageObject result = future.get(); // This will throw an exception if the future completes exceptionally
    ...
    void containerOptionsBuilder_shouldCreateOptions() {
        // Act
        ContainerOptions options = service.containerOptionsBuilder()
                .publicAccess(true)
    ...
    void objectOptionsBuilder_shouldCreateOptions() {
        // Act
        ObjectOptions options = service.objectOptionsBuilder()
