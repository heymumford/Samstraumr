...
class MigrationFeedbackTest {
    ...
    void testReportAndRetrieveIssues() {
        ...
        // Verify issue types
        List<MigrationIssue> typeMismatchIssues = MigrationFeedback.getIssuesByType(IssueType.TYPE_MISMATCH);
        List<MigrationIssue> missingPropertyIssues = MigrationFeedback.getIssuesByType(IssueType.MISSING_PROPERTY);
        List<MigrationIssue> stateTransitionIssues = MigrationFeedback.getIssuesByType(IssueType.STATE_TRANSITION);
        ...
        assertEquals("TubeStatus", typeMismatchIssue.getLegacyValue(), "Legacy value should be 'TubeStatus'");
        ...
        assertEquals("TERMINATED", stateTransitionIssue.getS8rValue(), "To state should be 'TERMINATED'");
    }
    ...
    void testGetStatistics() {
        ...
        assertEquals(2, countBySeverity.getOrDefault(Severity.WARNING, 0), "Should have 2 WARNING issues");
        ...
        assertEquals(1, countByType.getOrDefault(IssueType.TYPE_MISMATCH, 0), "Should have 1 TYPE_MISMATCH issue");
        assertEquals(1, countByType.getOrDefault(IssueType.MISSING_PROPERTY, 0), "Should have 1 MISSING_PROPERTY issue");
        assertEquals(1, countByType.getOrDefault(IssueType.REFLECTION_ERROR, 0), "Should have 1 REFLECTION_ERROR issue");
        assertEquals(1, countByType.getOrDefault(IssueType.LIFECYCLE, 0), "Should have 1 LIFECYCLE issue");
    }
    ...
    void testRecommendations() {
        ...
        boolean hasTypeMismatchRecommendation = recommendations.stream()
                .anyMatch(r -> r.getDescription().contains("TubeStatus") && r.getDescription().contains("State"));
        
        assertTrue(hasTypeMismatchRecommendation, "Should have a recommendation for TubeStatus to State conversion");
    }
    ...
    void testFilteringByMinimumSeverity() {
        ...
        logger.reportCustomIssue(IssueType.DEPENDENCY, Severity.DEBUG, "Debug issue - should be filtered");
        ...
        boolean allHighSeverity = issues.stream()
                .allMatch(issue -> issue.getSeverity().isAtLeast(Severity.WARNING));
    ...
    void testMaxIssuesPerComponent() {
        ...
        MigrationFeedbackConfig config = new MigrationFeedbackConfig.Builder()
                .setMaxIssuesPerComponent(2)
                .build();
        MigrationFeedback.configure(config);
        
        // Report 5 issues for the same component
        for (int i = 0; i < 5; i++) {
            logger.reportCustomIssue(IssueType.TYPE_MISMATCH, Severity.WARNING, 
                    "Issue " + (i + 1));
        }
