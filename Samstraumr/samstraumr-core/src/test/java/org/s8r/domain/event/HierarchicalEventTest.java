...
/**
 * Tests for the hierarchical event propagation system, validating that events properly
 * propagate through the inheritance hierarchy to appropriate handlers.
 * 
 * <p>This test suite demonstrates the inheritance-based event propagation system,
 * where handlers registered for parent event types receive events of child types.
 */
@UnitTest
...
public class HierarchicalEventTest {
    ...
    class BasicEventPropagationTests {
        ...
        void baseHandlersShouldReceiveDerivedEvents() {
            ...
            assertEquals(1, baseEventHandler.getReceivedEvents().size(), 
                    "Base event handler should receive the event");
            assertEquals(1, componentEventHandler.getReceivedEvents().size(), 
                    "Component event handler should receive the event");
            assertEquals(1, createdEventHandler.getReceivedEvents().size(), 
                    "Created event handler should receive the event");
            assertEquals(0, stateChangedEventHandler.getReceivedEvents().size(), 
                    "State changed event handler should not receive the event");
            
            // Verify the event is correctly passed to each handler
            assertSame(event, baseEventHandler.getReceivedEvents().get(0), 
                    "Base handler should receive the exact event instance");
            assertSame(event, componentEventHandler.getReceivedEvents().get(0), 
                    "Component handler should receive the exact event instance");
            assertSame(event, createdEventHandler.getReceivedEvents().get(0), 
        ...
        void stateChangedEventsShouldPropagateCorrectly() {
            ...
            ComponentId componentId = ComponentId.create("test-component");
            org.s8r.domain.lifecycle.LifecycleState oldState = org.s8r.domain.lifecycle.LifecycleState.INITIALIZED;
            org.s8r.domain.lifecycle.LifecycleState newState = org.s8r.domain.lifecycle.LifecycleState.RUNNING;
            ComponentStateChangedEvent event = new ComponentStateChangedEvent(
                    componentId, oldState, newState, "Test state transition");
            ...
            assertEquals(1, componentEventHandler.getReceivedEvents().size(), 
                    "Component event handler should receive the event");
            assertEquals(0, createdEventHandler.getReceivedEvents().size(), 
                    "Created event handler should not receive the event");
            assertEquals(1, stateChangedEventHandler.getReceivedEvents().size(), 
            ...
            assertEquals(componentId, receivedEvent.getComponentId(), 
                    "Component ID should match");
            assertEquals(oldState, receivedEvent.getPreviousState(), 
                    "Old state should match");
            assertEquals(newState, receivedEvent.getNewState(), 
    ...
    class AdvancedEventPropagationTests {
        ...
        void multipleEventTypesShouldBeHandledIndependently() {
            ...
            ComponentStateChangedEvent stateEvent = new ComponentStateChangedEvent(
                    componentId, org.s8r.domain.lifecycle.LifecycleState.INITIALIZED, 
                    org.s8r.domain.lifecycle.LifecycleState.RUNNING, "Test transition");
            ...
            assertEquals(3, baseEventHandler.getReceivedEvents().size(), 
                    "Base event handler should receive all events");
            assertEquals(3, componentEventHandler.getReceivedEvents().size(), 
            ...
            assertTrue(baseEventHandler.getReceivedEvents().contains(createdEvent),
                    "Base handler should receive created event");
            assertTrue(baseEventHandler.getReceivedEvents().contains(stateEvent),
                    "Base handler should receive state event");
            assertTrue(baseEventHandler.getReceivedEvents().contains(connectionEvent),
        ...
        void unregisteringHandlersShouldStopEventDelivery() {
            ...
            assertEquals(0, componentEventHandler.getReceivedEvents().size(),
    ...
    class EventMetadataTests {
        ...
        void eventsShouldContainProperMetadata() {
            ...
            assertEquals("ComponentCreated", event.getEventType(), 
    ...
    private static class ComponentConnectionEvent extends ComponentEvent {
        ...
        public ComponentConnectionEvent(ComponentId sourceId, ComponentId targetId, String connectionType) {
            super(sourceId);
