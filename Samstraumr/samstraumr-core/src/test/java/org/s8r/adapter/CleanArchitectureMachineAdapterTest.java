 ...
 * 
 * <p>These tests verify that the adapters properly implement the port interfaces
 * and perform the necessary type conversions between domain and component layers.
 */
@UnitTest
public class CleanArchitectureMachineAdapterTest {
    ...
    public void testDomainMachineAdapter() {
        ...
        assertEquals(domainMachine.getId().getIdString(), machinePort.getId().getIdString(), 
                "IDs should match");
        assertEquals(domainMachine.getState(), machinePort.getMachineState(), 
    ...
    public void testMachineFactoryAdapter() {
        ...
        MachinePort machinePort = machineFactoryAdapter.createMachine(MachineType.DATA_PROCESSOR, "factory-test");
        ...
        assertEquals(MachineState.STOPPED, machinePort.getMachineState(), 
                "Initial machine state should be STOPPED");
                
        // Test the convenience method as well
        MachinePort machinePort2 = machineFactoryAdapter.createMachine("factory-test-2", "DATA_PROCESSOR");
        ...
        assertEquals(MachineState.STOPPED, machinePort2.getMachineState(), 
                "Initial machine state should be STOPPED");
    }
    ...
    public void testMachinePortStateChanges() {
        ...
        MachinePort machinePort = machineFactoryAdapter.createMachine(MachineType.DATA_PROCESSOR, "state-test");
        ...
        assertEquals(MachineState.RUNNING, machinePort.getMachineState(), 
    ...
    public void testComponentPortImplementation() {
        ...
        ComponentPort componentPort = machinePort; // Should compile since MachinePort extends ComponentPort
        
        assertNotNull(componentPort, "Component port should not be null");
        assertEquals(domainMachine.getId().getIdString(), componentPort.getId().getIdString(), 
    ...
    public void testDefaultImplementations() {
        // Create adapter from domain machine
        MachinePort machinePort = machineFactoryAdapter.createMachine(MachineType.DATA_PROCESSOR, "defaults-test");
        ...
        assertEquals(LifecycleState.INITIALIZED.name(), machinePort.getState(), 
