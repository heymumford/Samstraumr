 ...
 * 
 * <p>This test suite verifies that the package structure aligns with Clean Architecture
 * and follows the defined organizational patterns.
 */
@UnitTest
@Tag("architecture")
@Tag("adr-validation")
...
public class PackageStructureTest {
    ...
    private static final Path SRC_DIR = Paths.get("/home/emumford/NativeLinuxProjects/Samstraumr/Samstraumr/samstraumr-core/src/main/java/org/s8r");
    ...
    class FileOrganizationTests {
        ...
        void packageInfoFilesShouldExistInCorePackages() throws IOException {
            ...
            assertTrue(Files.exists(SRC_DIR.resolve("domain").resolve("package-info.java")), 
                "Domain layer should have package-info.java");
            
            // Check application packages
            assertTrue(Files.exists(SRC_DIR.resolve("application").resolve("package-info.java")), 
                "Application layer should have package-info.java");
            
            // Check infrastructure packages
            assertTrue(Files.exists(SRC_DIR.resolve("infrastructure").resolve("package-info.java")), 
                "Infrastructure layer should have package-info.java");
            
            // Check adapter packages
            assertTrue(Files.exists(SRC_DIR.resolve("adapter").resolve("package-info.java")), 
        ...
        void filesShouldFollowConsistentNamingPatterns() throws IOException {
            ...
            boolean hasComponentEntity = componentFiles.stream()
            ...
            boolean hasComponentService = serviceFiles.stream()
                .anyMatch(p -> p.getFileName().toString().endsWith("Service.java"));
            ...
            boolean hasRepository = portFiles.stream()
                .anyMatch(p -> p.getFileName().toString().endsWith("Repository.java"));
            
            assertTrue(hasRepository, "Port package should have *Repository.java interfaces");
            
            // Check naming patterns in infrastructure
            List<Path> repoImplFiles = findJavaFiles(SRC_DIR.resolve("infrastructure").resolve("persistence"));
            
            boolean hasRepoImpl = repoImplFiles.stream()
                .anyMatch(p -> p.getFileName().toString().startsWith("InMemory") && 
                               p.getFileName().toString().endsWith("Repository.java"));
            
            assertTrue(hasRepoImpl, 
    ...
    class PackageDepthTests {
        ...
        void packageDepthShouldNotExceedReasonableLimits() throws IOException {
            ...
            assertTrue(maxDepth <= 6, 
                "Maximum package depth should not exceed 6 levels, but found " + maxDepth);
        }
    }
    ...
    private void checkPackageCoherence(Path directory, String commonSuffix) throws IOException {
        ...
        long countRelated = files.stream()
            .map(p -> p.getFileName().toString())
            .filter(name -> name.contains(commonSuffix) || 
                         name.equals("package-info.java"))
            .count();
        
        double coherenceRatio = (double) countRelated / files.size();
        
        assertTrue(coherenceRatio >= 0.75, 
            "Package " + directory + " should have at least 75% related files, but found " + 
            (coherenceRatio * 100) + "%");
    }
}